...
> [!NOTE] 読者の疑問：エラーの伝播って、コードが重複して複雑になりませんか？
...
>
> その認識は非常に鋭く、半分正解です。C言語には他言語のような `try-catch` 機構（自動的なエラーの伝播）がないため、各層で愚直にエラー判定を書こうとすると、本質的な処理よりもエラー処理のコード量の方が多くなってしまうという問題（エラーハンドリングのボイラープレート化）が必ず発生します。
