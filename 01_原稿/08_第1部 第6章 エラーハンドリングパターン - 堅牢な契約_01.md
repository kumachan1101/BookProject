...
#### エラーの伝播と責任 (設計の三本柱とエラーハンドリングの関係)

エラーコードが下位層から上位層へ伝播していく際に、各層のリテラシーに合わせた適切な「翻訳（マッピング）」が行われている様子です。下位層で発生した物理的で具体的なエラー（例えば「I/Oエラー」など）が、上位層へ上がるにつれて、その層にとって意味のある抽象的なエラー（例えば「ストレージ保存失敗」など）へと変換されることで、システム全体の依存関係がコントロールされます。以下の図は、エラーが下から上へ伝わる際の意味的な変化を示しています。
...
> [!INFO] 読者の疑問：エラーの伝播って、コードが重複して複雑になりませんか？
> 「エラーが下位層で見つかったら、上位層までエラーをバケツリレーのように繋げていく設計になるとのことですが、そうなると各層で『エラーなら上へ返す』という同じようなエラー判定のコード（`if (err != SUCCESS) return err;` など）が至る所に溢れ、かえってコードが重複だらけになる気がします。この認識は合っていますか？」
>
> **はい、その認識は非常に鋭く、半分正解です。**
>
> C言語には他言語のような `try-catch` 機構（自動的なエラーの伝播）がないため、各層で愚直にエラー判定を書こうとすると、本質的な処理よりもエラー処理のコード量の方が多くなってしまうという問題（エラーハンドリングの定型コード化）が必ず発生します。
> しかし、その問題を解決するための「美しい一元管理手法（`goto cleanup` パターンや `errno` 方式）」が存在します。本章では、いかにしてコードの重複を防ぎながら堅牢なエラー伝播を設計するかを詳しく解説していきます。
...
最初のパターンは、エラーコードにモジュールプレフィックスを付与して **階層的に管理する** 手法です。複数モジュールが混在するシステムでエラーの発生源を即座に特定できるよう、体系的なエラーコード体系を設計します。
...
エラーコードの数値範囲がモジュールごとに割り当てられ、競合しないように管理されています。Userは-1000番台、Storageは-2000番台と明確に区分けされており、これにより障害発生時の切り分けが迅速かつ確実に行えます。
...
#### storage.h (ストレージモジュール専用のエラー名前空間)
```c
#ifndef STORAGE_H
#define STORAGE_H
...
`log_error` 関数内部での分岐ロジックでは、エラーコードの数値範囲をチェックするだけで、どのモジュールで発生したかを正確に特定し、切り分けることができています。
...
次は **戻り値でエラーコードを返し、実際のデータは出力パラメータで返す** パターンです。関数の成否と結果データを明確に分離することで、呼び出し側がエラーチェックを省略できない強制力のある設計を実現します。
...
この仕組みは、関数がメモリを確保し、そのアドレスを呼び出し元のポインタ変数に書き込む流れとなります。万が一の**失敗時**には、`*user_out = NULL` を代入することを保証し、呼び出し側が初期化されていないゴミデータをつかんで誤動作するのを防ぐ設計となっています。
...
#### user.h (戻り値と出力パラメータの物理的完全分離)
```c
// 抽象契約の定義：User_tの宣言 (不完全型)
struct User;
typedef struct User User_t;

// エラーコード定数の定義（呼び出し側も参照するため、user.h に置く）
// 【設計のポイント】エラーコードはインターフェースの一部。
// 「関数の戻り値として何が返るか」は契約の一部であるため、
// user.h に定義して呼び出し側と実装側の両方から参照できるようにする。
#define USER_SUCCESS           0
...
#### user.c (確実なポインタ初期化による事故防止)
```c
#include "user.h"  // USER_SUCCESS, USER_ERROR_* をここからインクルード
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
...
#### main.c (エラーチェックを強要する安全な制御フロー)
```c
#include <stdio.h>
#include <stdlib.h>
#include "user.h"  // USER_SUCCESS, USER_ERROR_INVALID_ID は user.h で定義
...
3つ目は、標準Cライブラリの`errno`に倣った **グローバルエラー状態** の管理パターンです。スレッドローカルなエラー変数を用いてエラー情報を持ち回ることで、関数シグネチャを変えずにエラー詳細を呼び出し元へ伝達します。
...
戻り値（NULL）というメインの通信路とは別の経路でエラー情報が伝わる様子を、ここでは **サイドチャネル** と呼んでいます。
...
#### file_module.h (戻り値の型を維持した拡張エラーインターフェース)
```c
...
#### file_module.c (スレッドセーフなエラー状態の格納)
```c
...
最後は、複数の層をまたいでエラーを適切に伝播させる **エラー伝播チェーン** のパターンです。低レベル層のエラーを上位層へ引き上げる際にコンテキストを付加しながら、各層の責任範囲を明確に保ったまま処理します。
...
#### storage.c (I/Oエラーをシミュレートするダミー実装)
```c
#include "storage.h"
#include <string.h>
#include <stdio.h>
...
#### user_profile.h (中間層の抽象的なエラー契約定義)
```c
// user_profile.h
...
#### user_profile.c (具象エラーから抽象エラーへのマッピング実行)
```c
