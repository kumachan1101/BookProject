...
以下の `calculate_total` 関数は上位モジュールのビジネスロジックであり、実装上は **本来 `test_order.c` の中ではなく、`order_calculator.c` などの別ファイルとして存在すべきもの** です。テスト対象コードとテストコードを物理的に分離することで、本番のビジネスロジックは抽象インターフェース (`ITax`) にのみ依存し、具体的な計算方法やテスト用モックの存在を一切知らない状態に保たれます（ここでは解説の流れをわかりやすくするため、テストコードと同じファイルに並べて記載しています）。
