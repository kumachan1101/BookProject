...
SOLIDの各原則は、個別に機能する独立した概念ではなく、相互に補完し合う密接な関係にあります。
...
#### simple_processor.c (全ての具象に直接依存した変更に弱い（もろい）初期状態 - 初期バージョン - 密結合状態)
```c
...
#### simple_processor.c (processor_run：入力処理の責任が混在)
```c
...
#### simple_processor.c (processor_run：解析処理の責任が混在 - 続き)
```c
...
#### simple_processor.c (processor_run：出力と終了処理が混在 - 完結)
```c
...
#### simple_processor.c (具象実装のスタブ：密結合版の実体)
```c
// ========================================
// 具象実装のスタブ（トレースのため残す）
// ========================================
...
char* file_io_read(const char* path) {
    ...
    return strdup("{\"id\":10, \"name\":\"InitialUser\"}"); // ダミーJSON：{"id":10, "name":"InitialUser"}
}
...
正常に動作はしますが、内部構造は変更に対して非常にもろい状態です。
...
[Processor] データ解析開始
  [JsonParser] データ '{"id":10, "name":"InitialUser"}' を解析中...
...
#### ✅ 原則適用後：SRPとDIPを適用し、部品化・抽象化された柔軟な構造
...
struct IIOVTable {
    ...
    char* (*read)(void* context, const char* path);
    // データの書き込み責任
    int (*write)(void* context, const char* path, ProcessedData* data);
    // インスタンス破棄の責任
...
char* io_read(IIO* io, const char* path) {
    ...
    return io->vtable->read(io->context, path);
}

int io_write(IIO* io, const char* path, ProcessedData* data) {
    if (!io || !io->vtable || !io->vtable->write) return -1;

    return io->vtable->write(io->context, path, data);
}
...
struct IParserVTable {
    ...
    ProcessedData* (*parse)(void* context, const char* raw_data);
    // データ解放責任：このパーサーが生成したデータの解放方法を知っている
    void (*free_data)(void* context, ProcessedData* data);
    // インスタンス破棄責任
    void (*destroy)(void* context);
};
// 抽象インターフェース構造体
...
ProcessedData* parser_parse(IParser* parser, const char* raw_data) {
    ...
    return parser->vtable->parse(parser->context, raw_data);
}
...
void parser_free_data(IParser* parser, ProcessedData* data) {
    ...
    if (parser->vtable && parser->vtable->free_data) {
        parser->vtable->free_data(parser->context, data);
    } else {
...
void parser_destroy(IParser* parser) {
    ...
    if (parser->vtable && parser->vtable->destroy) {
        parser->vtable->destroy(parser->context);
    }
    free(parser);
}
```
...
static char* file_io_read_impl(void* context, const char* path) {
    FileIOData* data = (FileIOData*)context;
    data->access_count++;
    printf("  [FileIO %d回目] ファイル '%s' から読み込み\n", data->access_count, path);

    return strdup("{\"id\":10, \"name\":\"FileUser\"}"); // ダミーJSON：{"id":10, "name":"FileUser"}
}
// 具象実装：writeメソッド

static int file_io_write_impl(void* context, const char* path, ProcessedData* d) {
    FileIOData* data = (FileIOData*)context;
    data->access_count++;
...
#### file_io_impl.c (具象データと振る舞いを結合するファクトリ: VTableと生成)
```c
// 具象実装：destroyメソッド

static void file_io_destroy_impl(void* context) {
    if (context) {
        FileIOData* data = (FileIOData*)context;
...
static ProcessedData* json_parser_parse_impl(void* context, const char* raw_data) {
    JsonParserData* parser_state = (JsonParserData*)context;
    parser_state->parsed_count++;
    printf("  [JsonParser %d回目] データ '%s' を解析中...\n", parser_state->parsed_count, raw_data);
    // 実際にはJSON解析ライブラリを使用
    ProcessedData* data = (ProcessedData*)malloc(sizeof(ProcessedData));
...
#### json_parser_impl.c (メモリ管理の詳細を隠蔽するVTableと生成関数: 解放と生成)
```c
// 具象実装：free_dataメソッド

static void json_parser_free_data_impl(void* context, ProcessedData* data) {
    JsonParserData* parser_state = (JsonParserData*)context;
...
static void json_parser_destroy_impl(void* context) {
    if (context) {
        JsonParserData* parser_state = (JsonParserData*)context;
