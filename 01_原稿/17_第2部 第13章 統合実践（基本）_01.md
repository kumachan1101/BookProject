...
#### 全ての具象に直接依存した変更に脆い初期状態

#### simple_processor.c (初期バージョン - 密結合状態)
```c
...
#### 物理的な操作と制御の流れが混ざり合った状態

#### simple_processor.c (続き: ロジックの混在)
```c
...
#### 具象パーサーを直接呼び出してしまう解析処理

#### simple_processor.c (続き: 解析責任の混入)
```c
...
#### 実行の流れと後始末が混然一体となった制御

#### simple_processor.c (続き: 出力と終了処理)
```c
...
#### 全モジュールで共有される安定したデータ構造

#### processed_data.h (共通データ型)
```c
...
#### 生成と破棄の責任をセットにした解析の契約

#### i_parser.h (Parserの抽象契約)
```c
#ifndef I_PARSER_H
#define I_PARSER_H
#include "processed_data.h"
...
#### 純粋な制御責任だけを持つ美しいプロセッサ

#### processor.c (制御ロジックの完成)
```c
#include "processor.h"
#include <stdlib.h>
#include <stdio.h>
...
#### 具象データと振る舞いを結合するファクトリ

#### file_io_impl.c (続き: VTableと生成)
```c
// 具象実装：destroyメソッド
...
#### 抽象を返す生成関数だけを見せる公開ヘッダ

#### json_parser_impl.h (具象Parserのファクトリ)
```c
...
#### メモリ管理の詳細を隠蔽するVTableと生成関数

#### json_parser_impl.c (続き: 解放と生成)
```c
// 具象実装：free_dataメソッド
