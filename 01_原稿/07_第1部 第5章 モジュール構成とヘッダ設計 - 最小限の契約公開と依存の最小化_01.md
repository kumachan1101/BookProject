...
前章までに、`static`による**責任**の局所化、**関数ポインタ**による**契約**を通じた**依存**の切断、そして**第4章 不完全型**で**型の隠蔽**（抽象化）を習得しました。

本章では、これらの技術を統合し、モジュール設計の**外部的な側面**、すなわち**ヘッダファイル**（`.h`）の**デザイン**に焦点を当てます。C言語において、**ヘッダファイル**こそが、モジュールが外部に対して提供する「**契約**」であり、モジュール間の**依存関係**を決定づける「**境界線**」です。

ヘッダを厳格に設計する目的は、モジュール間の**依存を最小化**することにあります。これにより、一つのモジュールの内部変更が、他のモジュールに**連鎖的な再コンパイル**を引き起こさない**疎結合**が達成され、ソフトウェアの**変更容易性**が決定づけられます。
...
これらは**すべてヘッダ設計の問題**です。本章では、これらの具体的な問題を解決する設計技法を学びます。
...
* **密結合（上）**: `main.c` が `logger.h` を使うだけで、間接的に `hardware_driver.h` まで依存してしまっています。
* **疎結合（下）**: 前方宣言と不完全型を使うことで、`main.c` は `sensor.h` の「インターフェース」のみに依存し、その先の実装詳細（`hardware_driver.h`）を知らずに済みます。

**読み方のガイド**:

* 上の図では矢印が連鎖していますが、下の図では点線（前方宣言）によって連鎖が断ち切られています。

**問題:**

```mermaid
graph TB
      subgraph "密結合な依存関係<br/>(連鎖的なInclude)"
      A["main.c"] --> B["logger.h"]
      B --> C["sensor.h"]
...
C言語には言語レベルでの「名前空間（Namespace）」が存在しません。そのため、**プレフィックス規約**（例：`Book_`, `UI_`）を用いて、各関数や型がどのモジュールに属するかを明示することが、**責任の境界**を引く第一歩となります。
これにより、名前の衝突を防ぐだけでなく、そのコードが「誰の管轄か」を開発者が一目で判断できるようになります。

### 2. 「契約」の安定：外部との約束を固定する

モジュールの外部に公開するAPI宣言をヘッダファイルに集約し、`typedef` などで抽象化されたインターフェースを構築します。
内部実装（`.c`ファイル）がどれほど変更されても、ヘッダという**契約**が安定していれば、利用側のコードを修正する必要はありません。これが、システム全体の**変更耐性**を高める核となります。

### 3. 「依存」の最小化：結合を緩やかに保つ

**前方宣言**や**IWYU（Include What You Use）原則**を徹底し、不要な `#include` を排除することで、モジュール間の**コンパイル時依存**を最小化します。
「ヘッダの中で別のヘッダを過剰に読み込む」ことを防げば、一部の修正が全体に波及する「再コンパイルの連鎖」を食い止め、**疎結合**なシステムを実現できます。
...
**目的**: ヘッダファイルが「自己完結」していることを保証し、どの順番でインクルードしても正しくコンパイルできるようにする。

**機能**:

1. センサーの状態確認（bool型を使用）
2. センサーの生成と破棄

**アーキテクチャ**: 依存関係を明示したヘッダ構成。

### 設計の意図

ヘッダファイルが必要な型定義（例えば `bool`）を含んでいないと、利用側で「おまじない」のように特定のヘッダを先に読み込む必要が生じます。これは隠れた依存であり、メンテナンスの妨げになります。**IWYU原則**（自分が使うものは自分でインクルードする）を徹底し、さらに実装ファイルで自ヘッダを最初に読み込むことで、この問題を機械的に検出・防止します。

### 自己完結したヘッダの構造

**この図が示すもの**:

ヘッダファイルが単体でコンパイル可能である状態（自己完結）と、そうでない状態の違いです。

**注目ポイント**:

* **検証プロセス**: `sensor.c` が `sensor.h` を一番最初にインクルードすることで、`sensor.h` が他のヘッダ（stdbool.hなど）に依存しているかどうかをコンパイラがチェックします。
...
`stdbool.h` をインクルードしていないため、このヘッダ単体では `bool` 型が認識できず、コンパイルエラーになる可能性があります。
**評価:** 利用側に「先に `<stdbool.h>` をインクルードしてください」という暗黙のルールを強いることになり、契約として不完全です。
...
`sensor.h` の欠陥をカバーするために、利用側で先に依存関係を解決しています。これは「偶然動いている」だけであり、脆い設計です。
...
実装ファイルの先頭で `sensor.h` をインクルードすると、ここで初めてコンパイルエラーが発生します。
...
**設計的意図:** 依存関係をヘッダ内に閉じ込め、利用側の負担を減らす（契約の完全性）。
...
自ヘッダを最優先でインクルードすることで、ヘッダの品質（自己完結性）を常に保証します。
まず、構造体と生成関数です。

**処理の内容:** `sensor.h` を最初にインクルードし、構造体を定義してメモリを確保します。

**設計的意図:** もしヘッダに依存漏れがあれば、このファイルがコンパイルできないため、問題を即座に検知できます（フェイルファスト）。
...
ヘッダが自己完結しているため、単独でインクルードしても安全です。
...
**目的**: センサーモジュールとロガーモジュール、ハードウェアドライバモジュール間の依存関係を整理し、不要な再コンパイルを防ぐ。
...
**アーキテクチャ**: センサーはハードウェアを利用するが、ロガーなどの利用者にはハードウェアの詳細を見せない（隠蔽する）。

### 設計の意図

「センサーを使う」ことと「センサーがどう実装されているか」は別問題です。ヘッダファイルで `include "hardware.h"` をしてしまうと、センサーを使う全ての人にハードウェアの知識（依存）を押し付けることになります。**前方宣言**を使うことで、この依存を断ち切り、ポインタとしての「参照」関係だけに留めます。
...
* **点線**: `sensor.h` は `struct Hardware` の中身を知らず、ポインタとしてしか扱いません。これにより `hardware_driver.h` への依存が消えています。
* `logger.h` は `sensor.h` しか知らず、`hardware_driver.h` の存在すら知りません。
...
`hardware_driver.h` をインクルードすることで、ハードウェアの詳細が `Sensor` 構造体を通じて外部に漏れ出しています。
**評価:** `logger.c` はセンサーを使いたいだけなのに、ハードウェアドライバの変更の影響を受けてしまいます。
...
`sensor.h` をインクルードすると、見えないところで `hardware_driver.h` もついてきます。
...
**処理の内容:** ハードウェアを初期化し、ハンドルを返します。

**設計的意図:** 物理デバイスのセットアップをシミュレートしています。

**評価:** スタブとしての役割を果たしています。
...
**設計的意図:** データの取得処理をシミュレートしています。

**評価:** ダミー値を返すことでテストを容易にしています。
...
**不完全型**を用いることで、`hardware_driver.h` のインクルードを回避しています。
**設計的意図:** `sensor` モジュールの利用者はセンサーが内部でハードウェアを使っていることを知る必要がないため、その情報を隠蔽しています。

**評価:** 依存関係がクリーンになり、ビルド時間が短縮されます。
...
ハードウェアドライバへの依存を、この `.c` ファイル一つだけに限定しています。
まず、内部構造体の定義です。

**処理の内容:** `Sensor` 構造体を定義します。これが隠蔽される具象型です。

**設計的意図:** 依存のスコープを最小化し、変更時の影響範囲を抑え込みます。

**評価:** `sensor.h` に構造体定義を書かないことで、`hardware_driver.h` への依存を `sensor.c` 内に封じ込めています。
...
**処理の内容:** ハードウェアを初期化し、センサーオブジェクトを生成します。

**設計的意図:** ハードウェア初期化の詳細を隠蔽し、利用者に意識させないようにします。

**評価:** 抽象度の高いAPIを提供しています。
...
**処理の内容:** ハードウェアからデータを読み取ります。

**設計的意図:** ハードウェア操作のラッパーとして機能します。
...
**処理の内容:** ハードウェアを閉じ、メモリを解放します。

**設計的意図:** 依存リソースの解放もここで行い、責任を全うします。
...
`sensor.h` をインクルードしても、余計な `hardware_driver.h` はついてこないため、ロガーモジュールはハードウェア変更の影響を受けません。
...
**処理の内容**: センサーから値を読み取り、ログに出力します。

**設計的意図**: `logger.h` と同様に、`Sensor_t` の不透明ポインタを受け取ることで、具体的なセンサー実装（`hardware_driver.h` 等）への依存を排除しています。
...
**目的**: ユーザー管理モジュールと製品管理モジュールのように、異なるモジュールで似たような名前の関数（create, destroyなど）を使いたい場合に、名前の衝突を避ける。
...
**アーキテクチャ**: プレフィックス（接頭辞）を用いて擬似的な名前空間を実現する。

### 設計の意図

C言語には名前空間（namespace）がありません。そのため、`create` や `init` といった一般的な名前をそのまま使うと、リンク時に衝突してしまいます。モジュール名に基づいたプレフィックス（`user_`, `product_`）を強制することで、この問題を構造的に回避し、コードの所属を明確にします。
...
`create` や `destroy` という一般的な名前をそのまま使っているため、これら両方をリンクしようとするとエラーになります。
**評価:** 再利用性が著しく低い設計です。
...
**悪い例の続き:** 別のモジュールでも同様にプレフィックスのない関数名（`create`, `destroy`）を使用しているため、`user.h` と同時にインクルードするとコンパイルエラー（シンボル衝突）が発生します。
...
1. **処理内容**: ユーザーモジュールの公開契約として、プレフィックス付きのAPIを定義しています。
2. **設計意図**: 名前空間の汚染を防ぎ、リンク時のシンボル衝突を回避することで、モジュールの独立性を保ちます。
3. **評価**: 大規模開発においても安全に統合できる、堅牢なインターフェース設計です。
...
**処理の内容:** `user_create` でメモリを確保し、初期化します。名前の長さチェックも含みます。

**設計的意図:** プレフィックス `user_` を内部実装でも徹底し、可読性を高めています。

**評価:** エラーハンドリング（長さチェック）も含まれた実用的な生成関数です。
...
**アクセサと破棄の実装:** データの取得やメモリ解放も、専用の関数を通じて行います。
...
**処理の内容:** `struct Product` を定義し、`product_create` で初期化します。

**設計的意図:** `product_` プレフィックスにより、`user.c` の定義と区別されます。
...
int main(void)
{
    ...
    if (u && p) {
        printf("User: %s bought Product: %s\n", 
               user_get_name(u), 
               product_get_name(p));
...
**実行結果:** プログラムを実行すると、以下のようにユーザー名と製品名が正しく表示されます。名前空間の分離により、名前の衝突は発生しません。
