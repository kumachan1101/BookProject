...
第2のパターンは、関数（メソッド）の呼び出し時に依存を外から渡す「関数引数注入」です。
データ変換システムを例に、まずはDIPに違反した（具象に依存した）設計から見てみましょう。

以下のコードでは、本来別々のモジュールに分かれるべきAES暗号化やZIP圧縮といった具体的な実装ロジックが、一つのファイルの中に混在してしまっています。
...
第3のパターンは、オブジェクト生成後にプロパティ（メンバ変数）を通じて依存を注入する「セッター（プロパティ）注入」です。
ユーザー登録システムの通知機能を例に、まずはDIP違反のコードを見てみましょう。

以下の `UserService` では、通知先がEmailとSMSであることを完全に把握しており、設定用の関数（`set_notifier_manual`）を通じて強引に具象実装のポインタを受け取るという、非常に脆い設計になっています。
