...
★死活監視とは？市という言葉は使わないで、分かりやすい共通言語にして。
ここでは、デバイスの状態を監視する **死活監視システム（Monitor）** と、デバイスの詳細な設定を行う**構成管理システム（Configurator）** の2つのユースケースを持つデバイス管理基盤を実装します。これらのシステムは、共通のデバイスを対象としつつも、監視用APIと重厚な設定用APIという全く異なる要求事項を持つクライアントがそれぞれ存在するという要件を持っています。
