...
実際の開発現場での設計は、理論通りにはいかないことが多いのが現実です。一つの問題を解決しようとすると別の原則が関わってきたり、複数の技術を同時に使いこなす必要が生じることも少なくありません。

この「応用編」では、実際の開発現場で直面しやすい2つの複雑な課題に挑みます。一つは **「状況に合わせて処理を自在に入れ替える方法」** 、もう一つは **「至る所で使用される共有リソース（ロガーなど）とどう付き合うか」** です。

一見難しく感じられるかもしれませんが、第1部で学んだ「道具」と第2部の「原則」を組み合わせることで確実な設計が可能になります。順を追ってコードをリファクタリングする過程を解説します。
...
まずは、原則適用前の密結合なパイプラインの構造図を見てみましょう。

#### 依存と知識の「密結合」（違反状態）

パイプライン処理全体を管理するコードが、具体的な処理ステップ（フィルタ処理、圧縮処理など）とその「処理順番」を完全に知ってしまっている状態です。これでは順序を変えたい場合、司令塔の内部ロジックを直接書き換えるしかありません。
...
#### ❌ 原則適用前：処理順序がハードコードされた変更に弱いパイプライン
#### pipeline_bad.c (順序がハードコードされ修正が困難なプロセッサ - 密結合なパイプライン)
```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
####  柔軟性が欠如した固定順序の実行結果 (原則適用前 (Bad) の出力)
```c
=== main開始 (原則適用前：密結合) ===
  [Log] Filter 実行中...
  [Log] Compressor 実行中...
...
#### pipeline_process_bad.c (固定された関数名が引き起こす修正地獄 - 修正地獄の元凶)
```c
char* pipeline_process_bad(const char* input) {
...
#### code_snippet_01.c (順序変更によって引き起こされる組み合わせ爆発)
```c
// 処理が2つの場合: 2パターン
...
#### ✅ 原則適用後：Compositeパターンによる動的な処理構成の実現

インターフェースを導入し、 **「何をどの順番で実行するか」をパイプラインの外（main）から注入** できるようにします。

#### 全体アーキテクチャ図

改善後のシステム全体を俯瞰する構造図です。ここには **2つのVTable** が存在し、それぞれ異なる役割を担っている点に注目してください。

*   **具象処理用VTable**（`filter_vtable`, `compressor_vtable`）: 個々の処理の「中身」を定義する
*   **Composite用VTable**（`pipeline_vtable`）: 複数の処理を束ねて「順に実行する」責任を定義する

どちらのVTableも同じ `IProcessor` という抽象契約を実現しているため、パイプラインの中にパイプラインを入れるような再帰的な構成も可能です。

```mermaid
graph TB
      %% Composition Root
      subgraph Root ["Composition Root (main.c)"]
          Main["main関数<br/>(全具象を知る唯一の場所)"]
      end

      %% 抽象層
      subgraph Abstract ["抽象層"]
          IProc["IProcessor<br/>(不完全型 + 抽象API)"]
          VTableDef["IProcessorVTable<br/>{process, destroy}"]
          IProc ---|"内部に保持"| VTableDef
      end

      %% 具象処理層
      subgraph Concrete ["具象処理層 (各処理の実体)"]
          direction TB
          FVT["filter_vtable<br/>(具象VTable①)"]
          CVT["compressor_vtable<br/>(具象VTable②)"]
          FImpl["filter_impl<br/>(フィルタ処理)"]
          CImpl["compressor_impl<br/>(圧縮処理)"]
          FVT ---|"関数ポインタ"| FImpl
          CVT ---|"関数ポインタ"| CImpl
      end

      %% Composite層
      subgraph Composite ["Composite層 (複合処理)"]
          PVT["pipeline_vtable<br/>(Composite用VTable③)"]
          PImpl["pipeline_process_impl<br/>(順に実行するだけ)"]
          PList["IProcessor* 配列<br/>(処理の順序リスト)"]
          PVT ---|"関数ポインタ"| PImpl
          PImpl ---|"順番に呼ぶ"| PList
      end

      %% 接続
      Main ==>|"1. 具象を生成"| Concrete
      Main ==>|"2. 配列で順序を決定"| PList
      Main ==>|"3. Pipelineを生成"| PVT
      PList -.->|"抽象経由で実行"| IProc
      FVT -.->|"実現"| VTableDef
      CVT -.->|"実現"| VTableDef
      PVT -.->|"実現"| VTableDef
      ...
      %% スタイル
      style Abstract fill:#fff3e0,stroke:#ff9800,stroke-width:2px
      style Concrete fill:#e3f2fd,stroke:#1565c0
      style Composite fill:#f3e5f5,stroke:#7b1fa2
      style Root fill:#e8f5e9,stroke:#2e7d32
```

**この図の読み方：**

1. **`main`関数（緑）** だけが全ての具象を知っており、部品の生成と順序の決定を行います
2. **抽象層（オレンジ）** の `IProcessor` と `IProcessorVTable` が、全ての部品を統一する「共通言語」です
3. **具象処理層（青）** の各VTable（①②）は、それぞれの処理の「中身」だけに責任を持ちます
4. **Composite層（紫）** のVTable（③）は、他の `IProcessor` を受け取って順に実行する「束ね役」です
5. 3つのVTable（①②③）はすべて同じ `IProcessorVTable` を実現しているため、**どれも同じ `IProcessor` として扱えます**
...
#### concrete_processors.h (具体的な処理プロセッサを作成する抽象契約 - 具象プロセッサ生成の宣言)
```c
...
#### concrete_processors.c (マクロを活用して大量のボイラープレートを削減した実装 - マクロによる一括生成)
```c
...
// i_processor.h で定義されている VTable を使用する
#include "i_processor.h"
...
\
static char* name##_impl(void* context, const char* input) { \
    name##_data_t* data = (name##_data_t*)context; \
    data->call_count++; \
    char buf[256]; \
...
\
static void name##_destroy(void* context) { \
    free(context); /* name##_data_t を解放 */ \
} \
\
...
IProcessor* name##_processor_create(void) { \
    ...
    d->call_count = 0; \
\
    /* 安全なAPIを通じてインスタンスを生成・バインド */ \
    IProcessor* p = processor_create(&name##_vtable, d); \
    if (!p) { free(d); return NULL; } \
\
    return p; \
}
// 具象プロセッサの実装を展開
...
> [!NOTE] 補足：コンポジット（Composite）パターンとは？
> 「個別のオブジェクト（例：ファイル）」と「それらを束ねたオブジェクト（例：フォルダ）」を、使う側から見て「全く同じ1つのもの」として扱えるようにする設計パターンのことです。今回は、「単一の処理」と「複数の処理を束ねたパイプライン」をどちらも同じ `IProcessor` として扱えるようにしています。
...
菱形付き矢印: 集約（Aggregation）。PipelineがProcessorを持っていますが、ライフサイクルは管理しない（参照のみ）ことを示唆しています。
...
#### i_processor.h (不完全型で実装の詳細を隠蔽した最高レベルの抽象契約)
```c
...
/* 実装者が利用するVTableの定義 */
typedef struct {
    char* (*process)(void* context, const char* input_data);
    void (*destroy)(void* context);
} IProcessorVTable;

/* 抽象API：VTableを隠蔽し、不透明ポインタとして扱う */
/* 生成用関数：適切なVTableとコンテキストを紐付けて生成する */
IProcessor* processor_create(const IProcessorVTable* vtable, void* context);
...
#### i_processor.c (安全な実行と破棄を保証する抽象APIの実働部 - 抽象APIの実装)
```c
#include "i_processor.h"
struct IProcessor {
    const IProcessorVTable* vtable;
    void* context;
};

char* processor_execute(IProcessor* p, const char* input_data) {
    if (p && p->vtable && p->vtable->process) {
        return p->vtable->process(p->context, input_data);
    }
...
> [!INFO] 読者の疑問：なぜ context を VTable 経由で渡すと汎用的なのか？
> 実行関数の中で、`self`（`p`）全体ではなく `p->context`（その処理固有のデータ）のみを引数に渡している点に注目してください。これにより、VTable側（各々の具体的な処理の実装）は共通の `IProcessor` 構造体の全体構成を知らなくても済みます。また、固有データを `void*` に隠蔽して持たせ、呼び出された具象関数内で元の型（元の構造体）に安全にキャストして利用できるため、どんなデータ構造でも扱える極めて高い汎用性が生まれます。極端な話、状態を持たない関数の場合は `context` に `NULL` を渡し、単に「関数ポインタを直接テーブルに持たせて切り替える」だけの軽量な動的結合パターンとしても利用可能です。

いよいよ本丸です。「複数のプロセッサをまとめたもの（パイプライン）」自体も、また一つの `IProcessor` として振る舞うようにします。これが **Composite（複合体）パターン** です。

#### pipeline_processor.h (複数のプロセッサを一つに束ねる複合処理の契約 - Compositeのヘッダ)
```c
...
#### pipeline_processor.c (与えられた処理を順に実行する複合体の実装 - Compositeの実装)
```c
...
static char* pipeline_process_impl(void* context, const char* input_data) {
    PipelineProcessorImpl* p = (PipelineProcessorImpl*)context;
    // 最初のデータコピー
...
> [!NOTE] 補足：データが引き継がれる仕組み
> 上記の `for` ループ内に注目してください。`processor_execute` の戻り値（処理が終わった新しいデータ）を `next_data` として受け取り、古いデータを `free` で破棄した上で、`current_data = next_data;` と変数を更新しています。これにより、ループが回るたびに「前のプロセッサの出力結果」がそのまま「次のプロセッサの入力」として順番に引き継がれていく（数珠つなぎになる）仕組みが実現されています。

#### pipeline_processor.c (自身の破棄と実体のバインドを行う複合体の生成)
```c
static void pipeline_destroy_impl(void* context) {
    if (context) {
        PipelineProcessorImpl* impl = (PipelineProcessorImpl*)context;
...
IProcessor* pipeline_processor_create(IProcessor** processors, size_t count) {
    ...
    // 安全なAPIを通じてインスタンスを生成・バインド
    IProcessor* p = processor_create(&pipeline_vtable, impl);
...
#### main.c (処理の順序と構成を自由に組み立てるコンポジションルート - 柔軟な組み立て)
```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void) {
    ...
    // sizeof(配列全体) / sizeof(要素1つ) で要素数を自動計算
    IProcessor* pipe_1 = pipeline_processor_create(procs_1, sizeof(procs_1) / sizeof(procs_1[0]));
    ...
    IProcessor* pipe_2 = pipeline_processor_create(procs_2, sizeof(procs_2) / sizeof(procs_2[0]));
...
> [!INFO] 読者の疑問：結局誰かが具象（本物）を知らなきゃいけないのでは？
...
>
> **はい、その通りです。誰かが必ず具象を知る必要があります。ただし、それは「特定の1か所だけ」に限定されるべきです。**
>
> システムを動かすためには必ずどこかで「具象」を生成し、組み合わせる（結線する）必要があります。この「システム全体の依存関係を解決し、部品を組み立てる特別な場所」のことを **Composition Root（コンポジション・ルート）** と呼びます。
...
#### 実行結果 (柔軟に順序が組み替えられた実行結果)
```c
=== main開始 (原則適用後：柔軟な結合) ===
  [Log] Filtered 実行中... (呼び出し回数: 1)
  [Log] Compressed 実行中... (呼び出し回数: 1)
パイプライン1 結果: Compressed(Filtered(Data-1))

  [Log] Compressed 実行中... (呼び出し回数: 2)
  [Log] Filtered 実行中... (呼び出し回数: 2)
パイプライン2 結果: Filtered(Compressed(Data-2))

=== main終了 ===
```

#### なぜマクロを使うと楽になれるのか (C言語の隠れた武器：マクロで「面倒くさい」を自動化する)
...
#### NoMacro_Example.c (全てを手作業で記述した場合の冗長でもろいコード - マクロなしの冗長なコード)
```c
// Filter の実装

typedef struct { int call_count; } filter_data_t;

static char* filter_impl(void* context, const char* input) {
    filter_data_t* data = (filter_data_t*)context;
    data->call_count++;
    char buf[256];
    sprintf(buf, "Filtered(%s)", input);
...
static void filter_destroy(void* context) {
    free(context);
}
...
IProcessor* filter_processor_create(void) {
    ...
    // 安全なAPIでバインド
    IProcessor* p = processor_create(&filter_vtable, d);
    if (!p) { free(d); return NULL; }
...
#### NoMacro_Example.c (マクロを使用しない場合の冗長な実装例)
```c
// Compressor の実装（ほぼ同じ構造を再度記述）

typedef struct { int call_count; } compressor_data_t;

static char* compressor_impl(void* context, const char* input) {
    compressor_data_t* data = (compressor_data_t*)context;
    data->call_count++;
    char buf[256];
    sprintf(buf, "Compressed(%s)", input);
...
static void compressor_destroy(void* context) {
    free(context);
}
...
IProcessor* compressor_processor_create(void) {
    ...
    // 安全なAPIでバインド
    IProcessor* p = processor_create(&compressor_vtable, d);
    if (!p) { free(d); return NULL; }
...
#### Macro_Usage.c (マクロを利用した極めて簡潔で安全なプロセッサの生成 - マクロの利用)
```c
DEFINE_PROCESSOR(filter, "Filtered")
DEFINE_PROCESSOR(compressor, "Compressed")
```
...
マクロが「展開」されたあとの姿は、先ほど「マクロなしの冗長なコード」として見たNoMacro_Example.cの構造そのものです。つまりマクロは、あの冗長なコードをプリプロセッサが自動で書いてくれる仕組みに過ぎません。エラーや警告が出た場合は、この「展開後の姿（＝ノーマクロ版のコード）」を頭に思い浮かべるのがデバッグのコツです。
...
トークン結合による自動生成の仕組みです：
**`name##_data_t`** : `name` と `_data_t` を結合 → 固有のデータ構造体を生成する
**`name##_impl`** : `name` と `_impl` を結合 → `filter` を渡すと `filter_impl` 関数になる

このようにマクロ内で複数のトークン結合（`##`）が使われているのは、一つの完成された「部品（構造体定義、実装関数、破棄関数、VTable、生成関数のセット）」を一気に作り上げるためです。そして、生成された `name##_impl` の中では、渡された `void* context` を `name##_data_t*` にキャストし、その部品固有のデータ（例: 呼び出し回数）を安全に取り出して利用できる仕組みを持たせています。

それでは、先ほど `concrete_processors.c` で定義したマクロの中身を改めて見てみましょう。全く同じコードですが、ここでは `##` の働きに注目してください。

#### マクロ定義の再掲（concrete_processors.c より抜粋）
```c
#define DEFINE_PROCESSOR(name, label) \
\
typedef struct { int call_count; } name##_data_t; /* 具象専用の状態を定義 */ \
