...
データの持ち方（状態管理担当の要求）を変えると表示ロジックも壊れ、一方で出力をJSON形式に変えたい（UI担当の要求）と思うだけで、全く関係のないメモリ管理コードの動作保証テストまでやり直さなければならない、非常に脆く保守性の低い設計です。しかも `print_status` 関数が内部配列に直接アクセスしているため、将来管理構造をハッシュマップや木構造に変更することが事実上不可能です。
