...
#### Macro_Usage_Example.c (わずか1行の追加で完了する新しい処理の拡張 - 新しい部品の追加)
```c
// たった1行追加するだけ
...
#### Client_Usage_Example.c (マクロの存在を意識させないクリーンな利用側コード - 使う側のコード)
```c
...
ここまでの技術（抽象化、Compositeパターン、マクロによる量産）をフル動員して、5つの処理を自由につなぎ合わせるシステムを構築する過程を解説します。
ここで注目してほしいのは、処理の組み合わせ（ビジネスルール）が、もはや「ロジック」ではなく`main`関数内の「単なる配列のデータ（設定）」に変わっている点です。このレベルまで来ると、仕様変更に対する恐れは完全に消え去ります。

#### flexible_pipeline_demo.c (処理の無限の組み合わせを可能にする究極の柔軟性)
```c
/* 必要なヘッダのインクルード */
...
/* マクロの定義（前章の concrete_processors.c と同様のものを再掲） */
// ※本来、このマクロは共通ヘッダに定義して各ファイルから共有します。ここではコードを省略します。
...
#### flexible_pipeline_demo.c (構成ルートにおける柔軟な組み合わせの設定)
```c
int main(void) {
    // 各プロセッサを生成
...
#### Combinations.c (無限の組み合わせを配列の定義だけで実現 - 無限の組み合わせ)
```c
// 120通りのパターンすべてに対応できる
...
> [!WARNING] 注意：パラメータの汎用性と同期実行の前提
> これらの組み合わせが可能なのは、すべての処理が `IProcessor` という同一のインターフェース（`char* (*process)(void* context, const char* input_data)`）を守っているからです。
> 
> 「でも、画像リサイズ処理には『幅と高さ』、圧縮には『圧縮レベル』など、処理ごとに必要なパラメータが違うのでは？」と思うかもしれません。ここが `void* context` の真骨頂です。各処理用のパラメータは、プロセッサを生成する時点（`xyz_processor_create()`の内部）でそれぞれの `context`（固有のデータ構造体）内に保存しておきます。実行時（`process`呼び出し時）には、インターフェースの形を一切変えることなく `context` を通じて固有のパラメータを引き出せるため、全く異なる処理群であっても統一された配列として管理できるのです。
> 
> また、このパイプライン設計は基本的に **同期実行（前の処理が終わるのを待ってから次へ進む直列処理）** を前提としたシンプルなモデルです。もしネットワーク通信や重いファイルI/Oなどを多く含み、「非同期で並列にデータを流したい（非同期パイプライン）」といった要件がある場合は、単なる関数呼び出しループではなく、キュー（Queue）やスレッドワーカーを使った非同期アーキテクチャや、コールバック連鎖などを別途検討する必要があります。
...
#### Image_Pipeline.c (画像処理における柔軟なパイプラインの応用例 - 画像処理での応用)
```c
// 各処理に必要な固有のパラメータを渡してプロセッサを生成する（内部でcontextに保存される）
IProcessor* リサイズ = resize_processor_create(800, 600); // 800x600にリサイズ
IProcessor* フィルタ = filter_processor_create(FILTER_SEPIA); // セピア色フィルタ
IProcessor* 透かし = watermark_processor_create("Copyright 2026");
IProcessor* 圧縮 = compress_processor_create(9); // 圧縮レベル9
IProcessor* 最適化 = optimize_processor_create(true); // ロスレス最適化を有効に

// サムネイル生成用パイプライン
// ※本章の複合体（pipeline_processor）は同期実行（順次処理）ですが、
// 非同期実行が必要な場合は、生成した配列を専用のキューやワーカースレッドに渡して制御します。
...
データ分析の世界でも同様です。正規化や外れ値除去などの前処理は、順序を変えて何度も試行錯誤（実験）を繰り返します。そんなとき、配列の順番を変えるだけでパッと実験用と本番用を切り替えられるこの設計は、変更に強く、迅速な対応を可能にします。

#### Data_Analysis_Pipeline.c (データ分析における試行錯誤を支えるパイプライン構造 - データ分析での応用)
```c
...
> [!INFO] コラム：変数名での日本語使用について
> サンプルコードで `リサイズ` や `正規化` といった日本語を変数名として使用していますが、「C言語で日本語が使えるの？」と疑問に思うかもしれません。実は、最新のC言語規格（C99/C11以降）をサポートする多くのコンパイラ（GCCやClangなど）では、UTF-8エンコーディングを使用することで識別子（変数名や関数名）に日本語を使用することが可能です。ここでは、「構成要素を部品として並べる」という直感的なイメージを強調するため、あえて分かりやすい日本語を採用しています。

> [!NOTE] 補足：データが次の処理へ引き継がれる仕組み
> 「順番に配列へ入れているだけで、本当にデータが次の処理へ引き継がれるのか？」という疑問もあるでしょう。その仕組みは、前章で実装した **`pipeline_process_impl` 関数** の中に隠されています。司令塔となるパイプライン関数が、配列から処理を1つずつ取り出し、`char* next_data = processor_execute(p->processors[i], current_data);` のように「前の出力結果」を「次の入力」として渡し続ける（戻り値を数珠繋ぎにする）ループを回しているため、配列の順序通りにデータが自動的に引き継がれて変換されていくのです。
...
新しい順序パターンが必要になるたびに、関数を追加または修正する必要があります。処理が5つになると、120通りの関数が必要になります（事実上不可能）。既存コードへの変更が避けられず、バグ混入リスクが常に存在します。テストケースも組み合わせ数だけ必要になり、保守不可能です。
...
#### Comparison.c (適切な設計投資がもたらす圧倒的な保守性と拡張性 - 設計投資のリターン)
```c
...
第12章では、DIPの原則と共に「モック（テスト用の偽物の実装）」という概念を導入し、VTableを使った依存性の注入によってテスト時に本物とモックを差し替える基本的な手法を学びました。この課題2では、その知識を土台にして、実務で頻出する **「グローバル変数に直接依存しているせいでテストが書けない」** という、より複雑で現実的な問題に挑みます。第12章が「モックとは何か、どう作るか」を教えてくれる教科書だとすれば、本節は「現場で実際にどう使って戦うか」を体験する実戦演習です。
...
具体的なケースを用いて解説します。例えば、顧客データ管理システムにおいて、クレジットカード番号を暗号化して保存する機能を実装するとします。
...
#### test_impossible_example.c (本物の処理が走ってしまいテストが書けないコード)
```c
// テストコード
...
#### bad_implementation.c (隠れた状態と密結合によりテストを拒絶する実装 - テスト不能な密結合コード)
```c
/* ========================================
 * 具象的なAESエンジン（問題の根源）
 * ======================================== */
...
struct CustomerModule {
    int db_connection_id; // 例：データベースの接続IDなど
};
...
CustomerModule* customer_module_create(void) {
    ...
    if (self) {
        self->db_connection_id = 1001; // 仮想のDB接続ID
    }
...
void customer_save(CustomerModule* self, CustomerData* data) {
    ...
    printf("  [CustomerModule] DB(ID:%d)へ保存: 顧客 '%s', 暗号化カード '%s'\n",
           self->db_connection_id, data->name, encrypted);
    free(encrypted);
}
```
...
#### test_failure.c (どうやっても本物の重い処理が走ってしまうテスト - 強制的に本物が動くテスト)
```c
void test_customer_save() {
    CustomerModule* cm = customer_module_create();
...
#### usage_confusion.c (利用者からは裏側の依存が全く見えない危険な状態 - 見えない依存関係)
```c
CustomerModule* cm = customer_module_create();
...
graph TB
      ...
      subgraph InfraLayer ["下位層: ｲﾝﾌﾗｽﾄﾗｸﾁｬ"]
...
ステップ1です。まずは「暗号化」と「その解放」という振る舞いだけを約束する、抽象的なインターフェース（契約）を定義します。VTableなどの実装の裏側は不完全型で隠し、使う側には絶対に触らせません。これで、後からどんな中身（本物か、テスト用のモックか）でも安全に差し替えられるようになります。

#### i_crypto_engine.h (VTableを隠蔽し安全な交換を可能にする暗号化の契約 - 暗号化の抽象契約)
```c
...
#### i_crypto_engine_internal.h (具象実装を作る人だけがアクセスできる内部構造 - 実装者用の内部ヘッダ)
```c
...
typedef struct {
    char* (*encrypt)(void* context, const char* plain_text);
    void (*free_encrypted)(void* context, char* encrypted);
    void (*destroy)(void* context);
} ICryptoEngineVTable;
...
#### i_crypto_engine.c (利用者を安全に守る抽象APIのラップ実装 - 抽象APIの実装)
```c
#include "i_crypto_engine_internal.h"

char* crypto_encrypt(ICryptoEngine* engine, const char* plain_text) {
    if (engine && engine->vtable && engine->vtable->encrypt) {
        return engine->vtable->encrypt(engine->context, plain_text);
    }
...
void crypto_free_encrypted(ICryptoEngine* engine, char* encrypted) {
    if (engine && engine->vtable && engine->vtable->free_encrypted) {
        engine->vtable->free_encrypted(engine->context, encrypted);
    }
}
...
void crypto_destroy(ICryptoEngine* engine) {
    if (engine) {
        ...
        if (engine->vtable && engine->vtable->destroy) {
            engine->vtable->destroy(engine->context);
        }
        free(engine);
    }
}
...
#### customer_module.h (自身が必要とする依存を世界に宣言した抽象的な契約 - 依存を明示したヘッダ)
```c
...
#### customer_module.c (本物の処理の存在を知らずに抽象のみで動くロジック - 抽象にのみ依存するロジック)
```c
...
#### aes_crypto_adapter.c (状態を内部に隠蔽し契約通りに振る舞う本番用エンジン - 本番用の具象実装)
```c
...
static char* aes_encrypt_impl(void* context, const char* plain_text) {
    AESEngineData* data = (AESEngineData*)context;
...
static void aes_free_impl(void* context, char* encrypted) {
    // インターフェース（VTable）の型シグネチャに合わせるための引数ですが、
    // AESの解放処理では状態（context）を使わないため、未使用警告を抑制しています。
    (void)context;
    free(encrypted);
}
// 内部データの破棄責任を持つ

static void aes_destroy_impl(void* context) {
    if (context) {
        printf("  [AES] 内部データ破棄\n");
        free(context); // AESEngineDataの解放
    }
}
// VTableの実体
...
#### mock_crypto.c (実際の処理は行わず引数を記録するだけのスパイ実装)
```c
...
static char* mock_encrypt_impl(void* context, const char* plain_text) {
    MockCryptoData* data = (MockCryptoData*)context;
    data->call_count++;
    // 入力値を記録して検証可能にする
...
static void mock_free_impl(void* context, char* encrypted) {
    // VTableのシグネチャに合わせるための未使用引数警告の抑制
    (void)context;
    free(encrypted);
}

static void mock_destroy_impl(void* context) {
    // 【重要：安全設計】
    // MockCryptoDataはテスト関数側のスタック変数（または外部管理）へのポインタであるため、
    // ここで free(context) を実行してはならない（クラッシュの原因となる）。
...
ICryptoEngine* mock_crypto_create(MockCryptoData* shared_data) {
    ...
    engine->context = shared_data; // 外部データを参照として保持
...
#### main_production.c (本物のエンジンを注入して動作させる本番用の構成 - 本番環境での組み立て)
```c
#include "customer_module.h"
#include "aes_crypto_adapter.h"
#include <stdio.h>
...
#### test_customer_module.c (モックエンジンを注入して副作用なく検証する構成 - テスト環境での組み立て)
```c
#include "customer_module.h"
#include "mock_crypto.h"
#include <stdio.h>
...
この「 **DIP（依存の逆転） + 生成時注入 + モック** 」の組み合わせこそが、複雑なシステムを安全に構築するための重要な鍵です。外部依存（データベースやネットワーク通信など）が存在しても、この手法を使えばビジネスロジックだけを切り出して、確実にテストを行うことができます。これにより「依存関係が原因でテストが作れない」という課題を根本から解決できます。

#### 実行結果 (重い処理をバイパスし正確に呼び出しを記録した結果)
```c
=== テスト環境 ===
...
これらは、C言語でのシステム開発において「設計上の問題をどう解決するか」を考える際の、強力なフレームワーク（思考の枠組み）として活用できるパターンたちです。

#### 次章への橋渡し

いよいよ第2部の最終章です。これまで私たちは、各原則の意味から始まり、実際のコードを通じたパターンの使い方まで学んできました。

しかし、知識として知っていることと、現場で息をするように使いこなせることの間には、まだ少しの壁があります。次章「第15章：SOLID原則を『使える思考』にする」では、これまで学んだすべての知識を統合し、設計の現場で判断を下す際の「思考プロセス」そのものを言語化します。
