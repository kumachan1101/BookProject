...
コンパイル時にすべてが決定する **静的な結合** とは、例えば、上位の処理モジュールAが、下位の具体的なログ機能Bを **直接** 呼び出すような状況を指します。これは、上位モジュール（A）が下位の具象実装（B）に直接依存している、古典的で強固な結合状態です。Aのコードの中に「Bを呼ぶ」という記述が固定されているため、Bを後から別のものに差し替えるには、A自身のコードを書き換えるしかありません。
