...
#### 抽象インターフェースを生成するファクトリ（PostgreSQL）
 ...
 **シナリオ** ：環境モニタリングシステム。ハードウェアへの直接依存を排除し、単体テストを容易な設計にします。
...
#### 具象センサーをすべて知っているレポーター

#### weather_reporter.c (DIP違反)
```c
#include <stdio.h>
#include <stdlib.h>
...
#### 新しいデバイスが増えるたびに成長を続ける分岐地獄

#### weather_reporter.c (続き)
```c
// --- レポーター（複数の具象に密結合） ---
typedef struct {
