...
5. **「唐突な例示」の排除（ブリッジ文の必須化）**
   コード例（特に悪い例・良い例）を提示する際、「〜の例を示します。（悪い例）〜」といった機械的で唐突な導入は避けてください。「なぜここでこのコードを見るのか」がわかる自然な導入文（ストーリー）を必ず挟み、「（悪い例）」「（対策）」といったラベルは「なぜこれが悪い例と言えるのでしょうか。それは〜」のように平文に溶け込ませてください。
6. **項目間の「論理的な接着剤」の追加**
