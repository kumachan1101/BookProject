...
# ========= PDF処理（章ごとに分割） =========
def split_pdf_by_pages(pdf_path: Path, pages_per_chapter: int = 15) -> dict:
    """PDFを章ごとに分割して画像化"""
    if not pdf_path.exists():
        print(f"[WARN] PDFが見つかりません: {pdf_path}")
        return {}
    
    print(f"[PDF] {pdf_path.name} を章ごとに分割中...")
    ...
    try:
        ...
        for page_num in range(total_pages):
            ...
            if page_num % pages_per_chapter == 0:
                chapter_index = page_num // pages_per_chapter
            ...
            output_path = PDF_IMG_DIR / f"chapter{chapter_index:02d}_page{page_num % pages_per_chapter + 1:02d}.png"
            pix.save(str(output_path))
            optimize_image(output_path)
            chapter_images[chapter_index].append(f"images/pdf/{output_path.name}")
            print(f"  -> 章{chapter_index} ページ{page_num + 1}/{total_pages} 変換完了")
        
        doc.close()
        print(f"[PDF] {len(chapter_images)}章に分割しました")
...
chapter_pdf_images = split_pdf_by_pages(resource_pdf_path, pages_per_chapter=15)
...
def process_md(md_path: Path, chapter_index: int):
    ...
    if chapter_index in chapter_pdf_images:
        pdf_html = '<div style="margin: 2em 0; page-break-before: always;">\n'
        pdf_html += '<h2 style="font-size: 1.5em; margin: 1em 0; font-weight: bold; background-color: #fff3e0; padding: 0.6em 0.8em; border-left: 5px solid #ff9800;">参考資料</h2>\n'
