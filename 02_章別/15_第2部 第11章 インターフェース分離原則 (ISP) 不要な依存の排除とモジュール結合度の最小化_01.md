...
> [!NOTE] 読者の疑問：SRPで責任を分割すれば、ISPも自然に達成されるのでは？
> 「SRP（単一責任原則）をしっかり適用してモジュールの責任を分割すれば、インターフェースも自然と小さくなるはずです。わざわざISP（インターフェース分離原則）を別の原則として学ぶ意味はあるんですか？」
>
> 確かにSRPがきちんと適用されていれば、ISP違反は起きにくくなります。しかし、両者の **観点（視点の方向）** は明確に異なります。
> **SRPは「モジュールの内側」を見ます。** 「このモジュールの変更理由は一つか？」「内部に異なる責任が混在していないか？」という問いです。つまり、 **実装の凝集度** に関する原則です。
> 一方、 **ISPは「モジュールの外側（利用者との境界面）」を見ます。** 「このヘッダファイルやVTableは、利用者にとって不要な情報まで見せていないか？」「利用者ごとに、見せるべき窓口を分けるべきではないか？」という、 **アクター（利用者の役割）** に基づいた分割の問いです。
> したがって、SRPで内部を正しく分割しても、外部に公開するヘッダファイルが一つに統合されたままなら、利用者は不要な依存を強制されます。ISPは「誰に何を見せるか」という **公開範囲の設計** を律する原則なのです。
...
graph TB
      subgraph Interface ["1. 肥大化したinterface"]
          VT["IDeviceVTable<br/>(read / write /<br/>optimize)"]
      end
      ...
      %% 問題点の強調
      Problem["LSP違反: サポート外の<br/>writeやoptimizeの<br/>実装を強要されている"]
...
typedef struct {
    ...
    int (*optimize_db)(void* context);  // 特定のデバイスにしか関係ない契約
} IDeviceVTable;
```
...
#### ReadOnlyDevice.c (不要な実装を強要されLSP違反を犯すデバイス)
```c
// 読み取り専用デバイスの実装
static int read_only_read_data(void* context) {
    // 実際の読み取り処理
    return 0;
}

static int read_only_write_config(void* context) {
    /* * 【LSP違反の予兆】
...
graph TB
      ...
      subgraph Abstractions ["分離されたinterface"]
          direction TB
          VR["読込用 (IReader)"]
...
#### idevice_reader.h (読み取り能力だけに特化した最小の契約)
```c
...
#### idevice_configurator.h (設定の書き込み能力だけに特化した契約)
```c
...
#### idevice_optimizer.h (データベース最適化能力だけに特化した契約)
```c
#ifndef IDEVICE_OPTIMIZER_H
#define IDEVICE_OPTIMIZER_H

typedef struct {
    int (*optimize_db)(void* context);
} IOptimizerVTable;
#endif
```
...
#### IDeviceRWC.h (必要な能力だけを束ねるコンポジット構造体)
```c
...
typedef struct IDeviceRWC {
    void* context;                     // 具象データへの不透明ポインタ（第4章の応用）
...
#### ReadOnlyDevice.c (自分が提供できる能力だけを素直に実装するデバイス)
```c
#include "idevice_reader.h"
#include <stdio.h>
static int readonly_read_data(void* context) {
    // 読み込み専用なので状態（context）は使いませんが、
    // インターフェース（VTable）のシグネチャに合わせるために引数として受け取り、警告を抑制します。
    (void)context;
...
#### Client_Updater.c (複合されたポインタから安全に機能を利用するクライアント)
```c
...
void firmware_update_process(IDeviceRWC* device) {
    ...
    if (device->reader->read_data(device->context) != 0) {
        printf("Read failed\n");

        return;
    }

    if (device->config->write_config(device->context) != 0) {
...
> [!NOTE] 読者の疑問：アクターが分かれていなければ、インターフェースも分けなくて良い？
> 「もし『読み取り』と『書き込み』を両方とも必ず同じクライアント（一つのアクター）が使うシステムだったら、無理にISPを適用して分ける必要はないですよね？」
>
> **回答：**
> はい、その通りです！ISP（インターフェース分離原則）は「無意味にすべてを細かく分割せよ」という教条主義的なルールではありません。
> 「インターフェースの境界」は、常に「それを利用するクライアント（アクター）の境界」と一致すべきです。もしシステム上に「読み取りだけを行うクライアント」が存在せず、100%の確率で「両方セットで使う単一の巨大なアクター」しか存在しないことが確実であれば、インターフェースを一つにまとめて提供する方が自然であり、ISP違反にはなりません（YAGNI原則にも適っています）。
> ただし、将来的に「読み取り専用の別アプリ」が追加される可能性が少しでもあるなら、最初からVTableを役割ごとに分割し、コンポジション（複合）で提供しておく設計の方が、後々の変更コストを圧倒的に抑えられます。
