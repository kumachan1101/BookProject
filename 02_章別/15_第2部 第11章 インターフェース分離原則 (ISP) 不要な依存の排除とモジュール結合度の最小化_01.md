...
読み取り専用デバイスの実装です。書き込み機能はサポートしていないにもかかわらず、インターフェースを満たすためにダミーの関数（エラーを返すだけの実装）を用意しなければなりません。これはLSP違反の温床となります。
...
書き込み（設定）機能のみを定義した最小のインターフェースです。
...
**処理の内容:** `firmware_update_process` を呼び出すためのエントリーポイント(`main`)です。
**設計的意図:** 必要な機能(`IReader`, `IConfigurator`)の実装をコンポジションし、オブジェクトとして渡しています。
**評価:** クライアントは「自分が必要な機能」だけを持つオブジェクトを生成・利用できています。
