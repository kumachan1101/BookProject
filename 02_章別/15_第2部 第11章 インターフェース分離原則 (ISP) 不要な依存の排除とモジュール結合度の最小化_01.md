...
大規模プロジェクトにおいて、これはビルド時間を招くだけでなく、バイナリサイズの変化や予期せぬリンクエラーのリスクを生みます。「触っていない場所のせいでビルドが通らない」という状況は、開発サイクルを著しく停滞させます。
...
もし将来、デバイスの設定項目が増えて `device_types.h` が一行でも修正されたらどうなるでしょうか？設定機能には一切関心がない「監視担当」のプログラムまで、「あなたの依存先が変わったから」という理由で再コンパイルを強制されます。
