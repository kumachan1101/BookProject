...
>
> C言語でよく見られるアンチパターンに、システム中のあらゆるデータを1つの巨大な構造体（通称「神オブジェクト」）に詰め込んでしまう設計があります。例えば `struct AppContext` の中に `user_name` も `db_connection_status` も `screen_width` もすべてフラットに押し込むような状態です。これを行うと、画面サイズの変更という些細な修正だけでデータベース関連モジュールまで道連れに再コンパイルが発生したり、不要な権限を持つモジュールが誤って他のデータを壊してしまうリスクが跳ね上がります。ここから解説する「コンポジション」は、この巨大な構造体を意味のある「小さな責任の塊」に分割し、安全なブロック遊びのように組み合わせるための技術です。
...
本来、色は一度定義されたら変化しない「値」です。「赤」というオブジェクトの中身を書き換えて「青」にするのではなく、「青」という新しいオブジェクトを生成するべきなのです。この **不変性** により、データを安心して共有・再利用できます。
...
*   **書き換え不可**
    V1は点線で示され、変更されないことを表しています。
*   **新規生成**
    `color_to_grayscale` は新しいメモリ領域（V2）を確保して返します。
...
*   **隠蔽**
    クライアントは `malloc/free` を直接呼び出しません。
*   **責任の所在**
    メモリの確保と解放は、Entityモジュールが一手に引き受けています。
...
*   **モジュール**
    メモリの所有者であり、管理責任を持ちます。
*   **クライアント**
    利用者であり、生成・破棄の「依頼」のみを行います。
...
*   **責任の集約**
    `malloc`と`free`の詳細をモジュール内に閉じ込める
*   **抽象契約**
    クライアントは`create`/`destroy`のみに依存
*   **ライフサイクルの明確化**
    生成者が破棄の責任を持つ
...
「構造体が別の構造体を持つ」とき、最大の課題となるのが「誰がそのリソースを解放するのか？」という点です。このパターンでは、「自分が作ったもの（所有）は自分で解放し、人から借りたもの（参照）には手を出さない（解放しない）」という原則をコードで明確に表現します。
...
*   **コンポジション (所有)**
    `DeviceStatus` と `OwnedResource` はDeviceの一部であり、Deviceと共に運命を共にします。
*   **集約 (参照)**
    `Logger` は外部から借りてきたものであり、Deviceが消えても生き残ります。
