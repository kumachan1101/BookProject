...
前章までに、`static`による**情報隠蔽**（責任の局所化）と、第2章 関数ポインタと間接呼び出しによる**動的結合**（契約を通じた依存の切断）を学びました。これらは「**責任**」と「**振る舞い**」を設計する道具です。

本章の**構造体設計**は、設計の三本柱（責任・契約・依存）における **「責任」**、特に「データが持つべき責任」を明確にするための基本骨格となります。C言語の`struct`（構造体）に焦点を当て、データが持つ意味とライフサイクルに基づき、構造体を体系的に設計する手法を学びます。

### 構造体の三つの設計分類（概要）

構造体を設計する目的は、単にデータをまとめることではなく、その**構造体インスタンスがどのような種類の変更理由（責任）を持つべきか**を明確にすることです。

本書では、この**設計上の役割**と**ライフサイクル**に応じて、以下の三つの設計分類に大別します。この分類は、後に解説する**第8章 単一責任原則 (SRP)**の具体的な実践へと繋がります。

### データオブジェクトの分類

**この図が示すもの**:

構造体を役割に応じて3種類（Value, Entity, Service）に分類する考え方です。

**注目ポイント**:

* **責任の違い**: それぞれが管理すべき「責任」が異なります（値の整合性 vs ライフサイクル管理 vs 振る舞い）。

**読み方のガイド**:

* 3つの分類と、それぞれの責任（矢印の先）を対比してください。
...
Value Objectは、座標、色、時間といった「測定値や属性値」を保持するためのオブジェクトです。

* **役割と責任**: 役割を「純粋な値の保持」に限定します（**SRP：単一責任原則**）。
* **特性**: 最大の特徴は、中身の **「値（内容）」が同じであれば同一**とみなす点です。
* **設計判断のポイント**: 一度作成したら中身を書き換えず、新しい値が必要なら「新しい構造体」を生成する**不変性（Immutable）**を志向します。これにより、予期せぬ場所でデータが書き換えられる副作用を防ぎ、コードの予測可能性を高めることができます。

### 2. Entity Object（エンティティ）：寿命を管理する「意志」ある実体

Entityは、ユーザーやデバイスなど、独自のライフサイクル（生成・変更・破棄）を持つ実体を表します。

* **役割と責任**: 役割を「状態の管理」に限定します（**SRP：単一責任原則**）。
* **特性**: 値が変化しても、メモリ上の**「アドレス」や「識別子（ID）」**が同じであれば同一の存在として扱います。
* **設計判断のポイント**: 本書第7章で扱う「メモリ管理」において、**「誰が生成し、誰が破棄するか」という所有権とライフサイクルの責任**を明確に定義することが最も重要になります。

### 3. Service Object（サービスオブジェクト）：機能を仲介する「抽象」の存在

Service Objectは、ロガーやVTable（仮想関数テーブル）のように、具体的な処理やロジックを提供します。

* **役割と契約**: 内部状態を隠蔽し、外部へ「振る舞い」を公開します（**DIP：依存性逆転原則**）。
* **特性**: 特定のデータ内容ではなく、提供される機能（インターフェース）によって定義されます。
* **設計判断のポイント**: 第2章の関数ポインタや第4章の不完全型を組み合わせることで、呼び出し元に対して「どう実装しているか」という詳細を隠し、**「何をするか」という契約を保証**します。これにより、テスト容易性と実装の差し替え自由度を確保します。
...
このように、各オブジェクトが担う「責任」の所在と「契約」の形を意識することで、C言語による設計はより堅牢で、変更に強いものへと洗練されていきます。
本章は、主にデータの**責任**の明確化、特に**Value Object**と**Entity Object**の設計と、それらを組み合わせる**コンポジション**（合成）に焦点を当てます。

### コンポジション（合成）とは

実装イメージとしては、ある構造体が別の構造体を「メンバ変数として持つ」ことを指します。これにより、小さな部品を組み合わせて大きな機能を作ることができます。
特に重要なのは「親が壊れるときは子も一緒に壊れる」という**強い所有権の関係**を意味する点です。これは後の「ライフサイクル管理」で重要な意味を持ちます。
...
**目的**: 色情報（RGB）を扱うシステムにおいて、色の計算や変換を行う。
...
**アーキテクチャ**: データを不変（Immutable）として扱い、副作用のない関数で操作する。

### 設計の意図

色は一度定義されたら変化しない「値」です。「赤」というオブジェクトの中身を書き換えて「青」にするのではなく、「青」という新しいオブジェクトを作るべきです。この**不変性**により、データを安心して共有・再利用できます。
...
* 上から下へのデータの流れ。V1がOpに入力され、V2が出力される。
...
Value Objectとしての構造体定義と、その操作関数のインターフェースです。
**処理の内容:** `Color` 構造体を定義し、生成、変換、比較の関数を宣言しています。

**設計的意図:** `Color` 構造体は「値」を表現するため、メンバ変数（R, G, B）は隠蔽せず公開しています（Value Objectの特性）。変換関数 `color_to_grayscale` は、元の値を変更するのではなく、**新しい値を返す**ように宣言されており、不変性を意図しています。

**評価:** 安全で予測可能な値の操作を実現する契約です。
...
Value Objectに対する副作用のない操作の実装です。
まず、オブジェクトを生成するコンストラクタです。

**処理の内容:** 指定されたRGB値を持つ `Color` 構造体を返します。

**設計的意図:** 単純な初期化ですが、関数として提供することで構造体の内部表現が変わってもクライアントコードを守ることができます。

**評価:** 生成の責任をこの関数に集約しています。
...
**処理の内容:** 引数として渡された構造体を変更せず、計算結果を新しい構造体として返しています。

**設計的意図:** **不変性（Immutability）** の実現です。関数が副作用を持たないため、並行処理やデータの共有が安全に行えます。
...
**処理の内容:** 2つのColorオブジェクトの全メンバが一致するかを確認します。

**設計的意図:** アドレスではなく「中身の値」で等価性を判断する、Value Objectの根本的な性質を実装しています。

**評価:** 意図通りの等価性判定を提供しています。
...
Value Objectを利用するクライアントコードです。
まず、Value Objectの特性を確認する関数です。

**処理の内容:** 元の `Color` オブジェクトと変換後のオブジェクトが別物であることを確認しています。

**設計的意図:** 不変オブジェクトを使うことで、元のデータがいつの間にか変わってしまうというバグを防げることを示しています。

**評価:** 安心してデータを扱える設計効果が表れています。
...
**目的**: ユーザー情報を管理するシステムにおいて、ユーザーオブジェクトの生成から破棄までのライフサイクルを安全に管理する。
...
**アーキテクチャ**: メモリ管理の責任をEntityモジュールに集約し、クライアントから隠蔽する。

### 設計の意図

Entityは「同一性」と「寿命」を持つオブジェクトです。C言語ではメモリ管理が煩雑になりがちですが、生成(`create`)と破棄(`destroy`)をセットで提供し、その内部で `malloc`/`free` を完結させることで、クライアント側の負担とメモリリークのリスクを減らします。
...
Entity Objectのインターフェースと、ライフサイクル管理APIの定義です。
**処理の内容:** `User` 構造体と、`user_create`, `user_destroy` 関数を宣言しています。

**設計的意図:** 構造体の中身を見せていますが、本来Entityは中身を隠すべき（不完全型）です（注釈にもある通り）。重要なのは `create` と `destroy` のペアを提供し、利用者が直接 `malloc` / `free` しなくて済むように設計している点です。

**評価:** メモリ管理の責任をモジュール側に引き寄せるための第一歩となる設計です。
...
**処理の内容:** `create` で `malloc` し、初期化を行います。

**設計的意図:** メモリ操作という危険で低レベルな処理をこの関数内に閉じ込め、外部には安全なAPIだけを提供しています。

**評価:** 生成に関する責任が単一化されています（SRP）。
...
**処理の内容:** `user_destroy` で `free` しています。

**設計的意図:** 解放漏れを防ぐため、確保したモジュール自身が解放の手順を提供します。NULLチェックなどもここで行います。

**評価:** 安全なリソース解放を保証しています。
...
Entityのライフサイクルに従うクライアントコードです。
まず、Entityを利用する関数です。

**処理の内容:** 安全なAPI (`user_create`, `user_destroy`) を使って Entity を扱っています。

**設計的意図:** `malloc`/`free` の呼び出し忘れや二重解放といったミスを防ぐため、モジュールが提供するライフサイクル関数を利用しています。

**評価:** 安全なリソース利用の模範的なコードです。
...
生成と破棄が対で行われ、メモリが正しく管理されていることがログから確認できます。
...
**目的**: 複数の部品（内部リソース、外部リソース、値オブジェクト）を組み合わせて、一つのデバイスEntityを構成する。

**機能**:

1. デバイスの生成（内部リソースの確保と外部リソースの紐付け）
2. デバイスの破棄（所有しているリソースのみ解放）

**アーキテクチャ**: 親Entityが子要素のライフサイクルを管理するコンポジション構造。

### 設計の意図

「構造体が構造体を持つ」とき、一番の問題は「誰が誰を解放するのか？」です。このパターンでは、「自分が作ったもの（所有）は自分で解放し、人から借りたもの（参照）は放置する」という原則をコードで表現します。
...
コンポジションと集約（参照）が混在するEntityの定義です。
**処理の内容:** `Device` 構造体の中に、値としてのメンバ（`DeviceStatus`）、所有するポインタ（`Buffer`）、参照するだけのポインタ（`Logger`）を定義しています。

**設計的意図:** メンバ変数の持ち方によって「所有権」が異なることを表現しています。

**評価:** 構造体を見るだけでリソースの関係性が推測できる設計ですが、コメントによる補足も重要です。
...
以下は、このEntityのライフサイクルを管理する関数群の宣言です。
...
### device.c
複雑な所有権を持つEntityのライフサイクル実装です。
**処理の内容:** `device_destroy` 関数が核心です。自身が `malloc` した `internal_buffer` は解放し、外部から渡された `logger` は解放しません。

**設計的意図:** **「作った者が後始末をする」** という原則の徹底です。親オブジェクト（Device）が破棄されるとき、自分が所有している子オブジェクトも道連れにしますが、他人のものには手を出しません。

**評価:** 複雑なリソース管理を安全に行うための必須パターンです。
...
**処理の内容:** `malloc` で自身のメモリを確保し、子要素の初期化（値・参照・所有）を行います。

**設計的意図:** 自身の所有するリソース（`internal_buffer`）については責任を持って確保し、失敗時の巻き戻しもここで行います。

**評価:** 複雑な初期化手順を隠蔽し、整合性のある状態でのみオブジェクトを返却する設計です。
...
**処理の内容:** 自身の所有するリソース（`internal_buffer`）を解放し、最後に自身を解放します。

**設計的意図:** **「作った者が後始末をする」** という原則の徹底です。親オブジェクト（Device）が破棄されるとき、自分が所有している子オブジェクトも道連れにしますが、他人のもの（`logger`）には手を出しません。

**評価:** リソースリークを防ぐための明確な所有権モデルを示しています。
...
このコンポジション構造を利用するコードです。
**処理の内容:** `device_destroy` を呼んでも、注入した `dummy_logger` は破壊されないことを示しています。

**設計的意図:** リソースの寿命が異なるオブジェクト同士を組み合わせる際の、正しい生存期間管理を示しています。

**評価:** 所有権の境界を意識した正しい実装です。
...
1. **値ベースメンバー**（`DeviceStatus status`）

* 構造体に直接埋め込まれる
* 親の破棄時に自動的に解放される
* 明示的な解放処理は不要

2. **所有ポインタ**（`OwnedResource* internal_buffer`）

* 親Entityが`malloc`で生成し、破棄する責任を持つ（**Composition**）
* `device_destroy`内で明示的に解放

3. **非所有ポインタ**（`Logger* logger`）

* 外部で生成され、引数として渡されるリソースへの参照（**Aggregation**）
* 親Entityは破棄の責任を持たない