...
// デバイスの種類が増えるたびにここを修正しなければならない
typedef enum {
    DEVICE_SERIAL,
    DEVICE_ETHERNET
    // ここに DEVICE_USB を追加することになる
} DeviceType;

void print_data(DeviceType type, const char* data) {
    // 新しいデバイスが追加されるたびに、この分岐に手を入れる必要がある
    if (type == DEVICE_SERIAL) {
        printf("[Serial] Writing: %s\n", data);
    } else if (type == DEVICE_ETHERNET) {
        printf("[Ethernet] Sending: %s\n", data);
    }
    // ここに USB の条件分岐を追加することになる
}

int main(void) {
    print_data(DEVICE_SERIAL, "Hello");
    print_data(DEVICE_ETHERNET, "World");
    // 新しいデバイスを利用するコードを追加
    return 0;
}
```
...
シリアル通信と同様に、イーサネット通信の実装も作成します。それぞれの通信方式に応じた固有のデータ（IPアドレスなど）を `Context` に持ちます。

#### ethernet_device.c (イーサネット通信の実装)
```c
#include "idevice.h"
#include <stdio.h>
#include <stdlib.h>
...
static int ethernet_write(void* context, const char* data, size_t len) {
    EthernetContext* ctx = (EthernetContext*)context;
    printf("[Ethernet] IP %s: Sending '%.*s'\n",
           ctx->ip_address, (int)len, data);
    return (int)len;
}

static void ethernet_close(void* context) {
    EthernetContext* ctx = (EthernetContext*)context;
    printf("[Ethernet] Closing connection to %s\n", ctx->ip_address);
    free(ctx);
}

static const IDeviceVTable ETHERNET_VTABLE = {
    .write_data = ethernet_write,
    .close = ethernet_close
};

IDevice* ethernet_device_create(const char* ip_address) {
    EthernetContext* ctx = malloc(sizeof(EthernetContext));
    if (!ctx) return NULL;
...
=== Serial Device ===
[Core] データ処理中...
[Serial] Port COM1: Writing 'Hello, Device!'
[Serial] Closing port COM1

=== Ethernet Device ===
[Core] データ処理中...
[Ethernet] IP 192.168.1.1: Sending 'Hello, Device!'
[Ethernet] Closing connection to 192.168.1.1

=== USB Device ===
[Core] データ処理中...
[USB] Vendor ID:0x1234 Product ID:0x5678: Transmitting 'Hello, Device!'
[USB] Closing device Vendor ID:0x1234 Product ID:0x5678
```
...
static int apply_percent(void* context, int base_price) {
    PercentContext* ctx = (PercentContext*)context;
...
static int apply_fixed(void* context, int base_price) {
    FixedContext* ctx = (FixedContext*)context;
...
この壊れやすい結合を断ち切り、OCP（修正に対して閉じ、拡張に対して開く）を実現するための手法が、 **「VTableパターン（ストラテジーパターン）」の適用** です。
...
#### ✅ 原則適用後：演算子を独立したモジュールとして拡張可能にした構成
...
static int calc_credit(void* context, int price) {
    CreditContext* ctx = (CreditContext*)context;
...
static int calc_convenience(void* context, int price) {
    ConvenienceContext* ctx = (ConvenienceContext*)context;
