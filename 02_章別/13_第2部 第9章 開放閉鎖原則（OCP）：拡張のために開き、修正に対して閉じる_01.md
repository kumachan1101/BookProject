...
まず、割引計算ロジックが `if-else` の条件分岐で直接実装されているアンチパターンです。

「10%割引」や「500円引き」といった具体的な計算式が、1つの関数の中にハードコーディングされています。新しい割引ルールが増えるたびにこの関数を開いて分岐を追加しなければならず、OCP（修正に対して閉じる）に真っ向から違反している状態です。

#### 計算ルールがハードコードされた算出ロジック
...
まずは、よく見かけるものの非常に壊れやすいアンチパターンのコードです。

このファイルでは、支払い種別を表す `enum` の値（0, 1, 2...）をそのまま関数ポインタ配列のインデックスとして流用しています。一見すると `if` 文や `switch` 文がなくてスッキリしているように見えますが、これが罠です。「`enum` の定義順」と「配列要素の並び順」が暗黙的に強く結合してしまっています。

#### 順序の暗黙的ルールに依存したアンチパターン
