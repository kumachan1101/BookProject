...
この要件に対し、センサー値を読み取るインターフェースにおいて、クライアントは「呼び出すたびに最新の値が得られる」という **振る舞いの契約** （不変条件）を期待しています。しかし、ある具象実装（以下の連携コードにある `EcoSensor.c`）が、 **「消費電力削減のため、内部で500msキャッシュする」** という最適化を勝手に行うケースが多々見られます。
これは単体で見ると「エラー」ではありませんが、LSPの観点からは致命的な **「不変条件（時間的ルールの）破壊」** となります。クライアントが高速な制御ループを期待して10ms周期で呼び出している場合、500ms間同じ古い値を返し続ける実装が渡されると、リアルタイム性が失われ、制御システムが暴走したり不安定になったりするという深刻な問題を引き起こします。
...
左側の**制御ループ**は頻繁に問い合わせを行っていますが、中央の**EcoSensor**が「キャッシュ」によって遮断壁となり、古い値を返し続けている様子に注目してください。
...
#### ❌ 原則適用前：不変条件を破壊する不適切な最適化
#### EcoSensor.c (具象の都合を優先し、契約（リアルタイム性）を破る実装)
```c
#include "isensor.h"
#include <time.h>
...
static float get_value_impl(void* context) {
    EcoSensor_Data* sensor = (EcoSensor_Data*)context;
...
#### main.c (リアルタイム性を期待する制御ループ)
```c
#include "isensor.h"
#include <stdio.h>

extern ISensor* create_eco_sensor(int id);

int main(void) {
    ISensor* sensor = create_eco_sensor(1);
    printf("--- 制御ループ開始 (10ms周期を想定) ---\n");

    for (int i = 0; i < 3; i++) {
        float value = sensor->vtable->get_value(sensor->context);
        printf("[Client] t=%dms: センサー値 = %.1f度\n", i * 10, value);
        // 実際には10ms待機して物理値が変化する想定
    }
    return 0;
}
```
...
```c
--- 制御ループ開始 (10ms周期を想定) ---
[Client] t=0ms: センサー値 = 30.0度
[Client] t=10ms: センサー値 = 30.0度 (更新されていない!)
[Client] t=20ms: センサー値 = 30.0度 (更新されていない!)
```
...
この問題を根本的に解決するための設計的観点は、 **「具象の独自の都合（最適化）よりも、インターフェースで約束された契約を絶対に優先する」** ことです。
もしクライアントが「常に最新」というリアルタイム性を期待するインターフェースを作ったのであれば、それに準拠するすべての具象実装は、キャッシュを行わずに常に直接ハードウェアから物理値を読み取る責任を持ちます。これにより、クライアントはどんなセンサーが来ても「契約通り最新値が返る」と安心して多態性を利用することができます。もしどうしても省電力なエコセンサーを使いたいなら、リアルタイムセンサーのふりをするのではなく、別の新たなインターフェースとして明確に作成し区別するのが正しい設計です。

#### ✅ 原則適用後：ファクトリ関数とカプセル化で不変条件を保護

この問題は、具象実装の独自の都合（省電力など）よりも、契約で定められた「最新値を返す」という時間的整合性を最優先することで根本的に解決します。具体的には、クライアントが期待する10msごとの制御周期に対し、実装側での500msキャッシュという勝手な最適化を排除し、常にハードウェアへアクセスする仕様に改めます。この不一致（キャッシュを優先する実装）は、制御系システムにおいて暴走や事故を招く重大なリスクとなりますが、具象が契約を遵守することで安全性を取り戻すことができます。もしどうしても遅い更新が必要な場合は、別のインターフェースを設けて明確に区別すべきです。
...
#### RealTimeSensor.c (契約を優先する誠実なポーリング実装)
```c
#include "isensor.h"
...
static float get_value_impl(void* context) {
    RealTimeSensor_Data* sensor = (RealTimeSensor_Data*)context;
...
extern ISensor* create_realtime_sensor(int id);

int main(void) {
    ISensor* sensor = create_realtime_sensor(1);
    printf("--- 制御ループ開始 (10ms周期を想定) ---\n");
...
--- 制御ループ開始 (10ms周期を想定) ---
[Client] t=0ms: センサー値 = 30.0度
[Client] t=10ms: センサー値 = 30.5度
[Client] t=20ms: センサー値 = 31.0度
```
...
- **契約の明示** : インターフェース定義に不変条件（キャッシュ禁止）を明記。もし要件上どうしても「キャッシュ（遅延更新）せざるを得ない」場合は、このリアルタイムインターフェースを無理に実装するのではなく、`ICachedSensor` といった別のインターフェースを新設し、利用者が「これは古い値を返す可能性がある」と明確に区別して扱えるように設計しなければなりません。
- **契約の履行** : 具象実装側は、独自の最適化（省電力）よりも共通の契約を優先して実装する。
- **信頼性の確保** : クライアントはどの実装が来ても「最新値である」と信じてコードを書くことができる。

### 3.3. 実践パターン3：事後条件の統一（エラーコードの誠実な返却）

処理が終わった後の状態（戻り値）も契約の一部です。クライアントが知らない「独自のエラーコード」を返すことは、事後条件を弱める（期待を裏切る）行為と言えます。
...
| **クライアントの判定** | `if (res < 0)` で統一判定可能 | **正の値を返すため正常終了と誤認** | **判定ロジック破綻** |
...
#### iresource.h (戻り値の意味を厳密に定義したリソース契約)
```c
#ifndef IRESOURCE_H
#define IRESOURCE_H
 ...
 */
typedef struct {
    int (*acquire_resource)(void* context);
    void (*release_resource)(void* context);
} IResourceVTable;
typedef struct {
...
#### ❌ 原則適用前：具象内部のエラーが漏洩し、クライアントに判断を強いる設計
#### NetResourceProvider.c (内部の事情をそのまま漏洩させる違反実装)
```c
#include "iresource.h"
#include <stdbool.h>
#include <stdio.h>
...
static int net_acquire_impl(void* context) {
    // 【解説】(void)context; は「未使用引数」の警告を抑制するためによく使われる書き方です。
    // この例ではモック関数しか呼ばないため context を使いませんが、
...
#### main.c (未知のエラーコードで破綻するクライアント)
```c
#include "iresource.h"
#include <stdio.h>

extern IResource* create_net_resource_bad(void);

int main(void) {
    IResource* res = create_net_resource_bad();
    printf("--- Acquiring Resource ---\n");
    
    int result = res->vtable->acquire_resource(res->context);
    ...
    if (result == SUCCESS) {
        ...
        if (result == ERROR_TEMPORARY) {
            printf("[Client] 一時エラー。リトライします...\n");
        } else if (result == ERROR_FATAL) {
            printf("[Client] 致命的エラー。システムを停止します。\n");
        } else {
            // ここに到達したということは、クライアントの想定外
            printf("[Client] 未知のエラーコード。判定ロジックが破綻しました。\n");
        }
    }
    return 0;
}
...
[Client] 未知のエラーコード。判定ロジックが破綻しました。
```
...
> [!NOTE] 補足：第6章のエラーハンドリングとの違い
> 第6章では、エラーコードをどのように定義し、伝播させ、上位レイヤーへマッピングするかという「実装の道具（仕組み）」について学びました。対して本章（LSP）における議論の焦点は、そのマッピングが「クライアントとの契約（約束）」を遵守しているかという「設計品質の保証」にあります。どれだけ優れたエラー処理の仕組みを持っていても、具象実装が契約外の（クライアントが知らない）エラーコードを返せば、それはLSP違反となり、多態性の安全性を根底から破壊してしまいます。第6章が「やり方」なら、本章は「あるべき姿」を論じています。
...
#### NetResourceProvider.c (内部エラーを契約コードへ翻訳するマッパー)
```c
#include "iresource.h"
#include <stdbool.h>
#include <stdlib.h>
...
#### NetResourceProvider.c (マッピングを経由して事後条件を守る実装)
```c
...
IResource* create_net_resource(void) {
    ...
    res->context = data; // 内部データ（コンテキスト）の注入
...
int main(void) {
    ...
    res->vtable->release_resource(res->context);
    free(res);
...
#### 1.  **LSPの本質を「置換可能性」の保証と定義する** : (リスコフ置換原則 (LSP) の設計指針)
...
- **不透明ポインタと不完全型（第4章）** : `void* context` や構造体の前方宣言により、具象の詳細をクライアントから物理的に隔離する。
