...
この実装ファイルでは、`get_value_impl` 関数内で独自のキャッシュ機構を導入しています。ハードウェアへのアクセスを減らして省電力を図るという意図は良いものの、「呼び出しごとに最新の値を返す」というインターフェースの暗黙の契約（不変条件）を破っており、リアルタイム性を必要とするクライアントに対して誤動作を引き起こします。
#### EcoSensor.c
```c
#include "isensor.h"
#include <time.h>
...
LSP違反（時間的整合性の破壊）の例を示すコードです。
**処理の内容:** 内部キャッシュを持ち、500ms以内なら古い値を返します。

**設計的意図:** （悪い例）省電力化という独自の最適化を優先し、契約を無視しています。

**評価:** 「常に最新」を期待するクライアントに対して、古い値を返すことで制御を破綻させます。
...
LSPを遵守した実装です。ここでは省電力などの独自の最適化を行わず、インターフェースの契約である「最新の値の返却」を最優先事項としています。これにより、クライアントは常に期待通りの動作を得ることができます。
...
センサーインターフェースの定義ファイルです。ここではコードだけでなく、コメント（契約書）として「不変条件」を明記しています。このドキュメンテーションこそがLSPを守るための第一歩となります。
...
LSPを遵守した実装です。内部エラーが発生した際、専用の `map_internal_error_to_contract` 関数を通して、契約で定義されたエラーコードに変換してから返却します。これにより、クライアントは内部エラーの詳細を知ることなく、統一されたロジックでエラー処理を行えます。
...
**クライアント実装:** 以下のコードは、ネットワークリソースを使用し、エラー発生時の挙動を確認します。LSPに準拠しているため、予期せぬエラーでクラッシュすることはありません。
...
**実行結果:** 内部のネットワークエラー（1001）が、契約で定義された一時的なエラー（-2）に変換され、クライアントに安全に伝わっていることがわかります。
