...
* **抽象化**: 下位の具体的なエラー（I/Oエラー）が、上位では意味のあるエラー（ストレージ失敗）に変換されています。
...
**目的**: 大規模システムにおいて、エラーコードだけで「どのモジュールで何が起きたか」を即座に判別できるようにする。
**機能**:

1. ユーザーモジュールのエラー定義
2. ストレージモジュールのエラー定義
3. エラーコードによる発生源の特定
**アーキテクチャ**: モジュールごとにエラーコードの数値範囲（名前空間）を分割する。
...
* **範囲**: Userは-1000番台、Storageは-2000番台と明確に区分けされています。
...
Userモジュールの公開契約として、-1000番台のエラーコードを定義します。
...
エラーコードを見るだけで、どのモジュールで問題が起きたかを判定できます。
...
* **範囲チェック**: エラーコードの数値範囲だけでモジュールを特定できています。
...
**実行結果:** 異なるエラーコードが返された場合でも、その範囲（-1000番台か-2000番台か）によって、どのモジュールで障害が発生したかを正しく判別できています。
...
**目的**: 関数の戻り値を「成功/失敗」の判定のみに使用し、生成されたデータは引数（ポインタ）経由で返すことで、エラーチェックを強制する。
**機能**:

1. ユーザーオブジェクトの生成
2. 生成成功時はポインタを返し、失敗時はエラーコードを返す
**アーキテクチャ**: 戻り値（エラーコード）と出力（データ）を分離したAPI設計。
...
* **失敗時**: `*user_out = NULL` を保証することで、呼び出し側がゴミデータをつかむのを防ぎます。
```mermaid
    graph TB
    %% 1. 呼び出し側
...
APIの戻り値を `int` とし、データは `User_t** user_out` で受け取ります。
...
失敗時に `user_out` を明示的に `NULL` にセットすることで、安全性を高めています。
**設計的意図:** もし呼び出し側が `user_out` を初期化していなくても、関数内で `NULL` にリセットすれば、未定義動作を回避できます。
...
**処理の内容:** `user_create` 内でメモリ確保と初期化を行います。失敗時は `user_out` を NULL にします。
**設計的意図:** もし呼び出し側が `user_out` を初期化していなくても、関数内で `NULL` にリセットすれば、未定義動作を回避できます。
**評価:** 安全なポインタ操作を保証しています。
...
**破棄処理:** メモリ解放を担当します。`NULL`チェックを忘れずに行います。
...
戻り値 `result` とデータ `user_handle` が分離されているため、エラー処理が自然に記述されます。
...
**処理の内容:** 成功ケースと失敗ケース（入力エラー）を検証します。
**設計的意図:** APIの正しい使い方とエラー時の挙動を実証します。
**評価:** 網羅的なテストケースとなっています。
...
**実行結果:** 入力エラーの場合は詳細メッセージが出力されますが、ハンドル自体はNULLであることが保証されています。これにより、呼び出し側での誤用を防げます。
...
**目的**: ファイル操作のような低レベルAPIにおいて、戻り値をポインタ（ハンドル）のまま維持しつつ、エラー詳細を通知する。
**機能**:

1. ファイルオープン
2. エラー詳細の取得
**アーキテクチャ**: スレッドローカルなエラー変数を介した状態共有。
...
* **サイドチャネル**: 戻り値（NULL）とは別の経路でエラー情報が伝わります。
...
`file_get_last_error()` を提供し、エラー発生時の詳細情報へのアクセスを許可します。
...
`_Thread_local` を使用して、並行実行時の安全性を確保します。
...
`_Thread_local` を使用して、並行実行時の安全性を確保します。
まず、エラー変数とファイルオープン関数です。

**処理の内容:** スレッドローカルなエラー変数を定義し、ファイル操作失敗時にそこに値を書き込みます。
**設計的意図:** グローバル変数の並行性問題を回避しつつ、errnoパターンの利便性を提供します。
**評価:** C11標準機能を用いた現代的な実装です。
...
終了処理とエラー情報の取得関数の実装です。
まず、クローズ関数です。

**処理の内容:** ファイルを閉じ、メモリを解放します。
**設計的意図:** リソースの確実な解放です。
**評価:** 安全な終了処理を提供しています。
...
**処理の内容:** スレッドローカルに保存されたエラーコードを返します。
**設計的意図:** サイドチャネルからの情報取得です。
**評価:** `errno`と同様のインターフェースです。
...
**処理の内容:** エラーコードに対応するメッセージ文字列を返します。
**設計的意図:** デバッグやログ出力に役立つ情報の提供です。
**評価:** 親切な設計です。
...
**処理の内容:** 正常系と異常系（ファイル不在、権限エラー）をテストします。
**設計的意図:** エラー時の詳細情報取得プロセスを実証します。
**評価:** 想定されるエラーケースを網羅しています。
...
**実行結果:** file_open 失敗時に、サイドチャネル（file_get_last_error）を通じて的確なエラー理由（ファイル不在、権限不足など）が伝わっていることがわかります。
...
* **マッピング**: User層がStorage層のエラーを受け取り、アプリ層向けのエラーに変換することで、アプリ層がStorage層の詳細を知らずに済むようにしています。
...
**構成図：** エラー伝播チェーン。各層が自身のエラーコード体系を持ち、下位層のエラーを適切にマッピングして上位へ伝達する。
...
**I/Oシミュレーション:** I/Oエラーをシミュレートするロジックを含みます。
まず、書き込み関数です。

**処理の内容:** 特定のキーでエラーを返します。
**設計的意図:** I/O障害のシミュレーション。
**評価:** テスト容易性を高めています。
...
**処理の内容:** バッファサイズをチェックし、データを書き込みます。
**設計的意図:** 安全なバッファ操作を示します。
**評価:** バッファオーバーフロー対策が含まれています。
...
下位の `STORAGE_ERROR_IO` を受け取り、上位向けの `USER_ERROR_STORAGE_FAILURE` に変換して返します。
**設計的意図:** 上位層に「保存に失敗した」ことだけを伝え、「ディスクが壊れた」のか「権限がない」のかといった詳細な理由（実装の詳細）は隠蔽します。
...
**処理の内容:** ストレージへの書き込みを行い、失敗した場合はエラーコードを変換（マッピング）します。
**設計的意図:** 上位層に「保存に失敗した」ことだけを伝え、「ディスクが壊れた」のか「権限がない」のかといった詳細な理由（実装の詳細）は隠蔽します。
**評価:** 依存区画を守る重要な翻訳層です。
...
**処理の内容:** ストレージから読み出し、同様にエラーマッピングを行います。
**設計的意図:** エラー処理の方針を統一します。
**評価:** 一貫性のあるエラーハンドリングです。
...
**処理の内容:** 正常系、入力エラー、ストレージエラーの伝播を確認します。
**設計的意図:** 階層化されたエラーハンドリングが機能しているか実証します。
**評価:** 適切なエラーコードが返ってくることを確認できます。
...
**実行結果:** 下位のストレージ層で発生したI/Oエラー（-2001）が、ユーザー層によって抽象的な保存失敗エラー（-1021）に変換され、アプリケーション層に通知されています。
