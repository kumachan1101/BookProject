...
「抽象に依存する」設計を徹底すると、一つ大きな疑問が生まれます。それは「誰が、どの具体的なオブジェクト（具象）を生成し、結びつけるのか？

」という点です。その役割を担うのがコンポジションルートです。
...
すべての具象クラスの生成と接続を `main` 関数（コンポジションルート）一箇所に集中させることで、アプリケーション全体の依存関係構造を俯瞰できるようにし、個々のクラスから「他のクラスを生成する責任」を取り除いています。
...
ここでは、`net_io_create`というファクトリ関数が存在し、それが`IIO`の契約通りに振る舞う抽象インスタンスを返すことを前提とします。`IIO` インターフェースのスタブ実装です。

このスタブ実装は、`IIO` インターフェースの契約（関数シグネチャと振る舞い）さえ満たせば、内部で何を行っても構わないことを示しています。これが「リスコフの置換原則（LSP）」の本質であり、モックテストの基礎となります。

契約さえ守れば実装は自由であることが検証できます。変更が発生しても、このファイル内のみに影響を留めることができます。
...
`main.c`内の具象生成部分を `FileIO` から `NetIO` へ切り替えます。ロジック本体には一切触れず、生成コードの変更のみでシステム全体の振る舞いを拡張できることが、OCP達成の証明となります。
