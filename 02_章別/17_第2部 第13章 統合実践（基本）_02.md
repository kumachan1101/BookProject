...
抽象インターフェースに対して、どの具象オブジェクトを流し込むかという「依存性の注入（DI）」を一箇所に集中させます。このように、別々に作られた部品（モジュール）同士をケーブルで繋ぐように結びつける作業を「**ワイヤリング（Wiring）**」と呼びます。これにより、アプリケーションの主要なロジック部分は、一切の具象を知ることなく、抽象のみに依存した状態で動作できるようになります。
...
#### main.c (全ての具象を知り依存を注入する構成責任者 - コンポジションルート)
```c
// システムのエントリポイント
...
graph TB
      ...
      subgraph Infrastructure ["3. ｲﾝﾌﾗｽﾄﾗｸﾁｬ"]
          direction LR
...
[Processor] 入力処理開始
  [FileIO 1回目] ファイル 'input.txt' から読み込み
[Processor] データ解析開始
  [JsonParser 1回目] データ '{"id":10, "name":"FileUser"}' を解析中...
  [JsonParser 1回目] 解析完了: ID=20, Name=FileUser_A
[Processor] 出力処理開始
  [FileIO 2回目] ファイル 'output.txt' へ書き込み: ID=20, Name=FileUser_A
  [JsonParser 1回目] データ解放
[Processor] 処理完了
  [JsonParser] リソース解放（総解析回数: 1）
  [FileIO] リソース解放（総アクセス回数: 2）
...
#### net_io_impl.c (OCPを検証するためのネットワーク操作スタブ - 検証用スタブ実装)
```c
#include "i_io.h"
#include <stdlib.h>
#include <stdio.h>
...
static char* net_read_impl(void* context, const char* path) {
    printf("  [NetIO] ネットワーク '%s' からデータ受信\n", path);
    // 実際はソケット通信などを行う
    return strdup("{\"id\":99, \"name\":\"NetworkUser\"}"); // ダミーJSON：{"id":99, "name":"NetworkUser"}
}

static int net_write_impl(void* context, const char* path, ProcessedData* data) {
    printf("  [NetIO] ネットワーク '%s' へ送信: ID=%d, Name=%s\n", path, data->id, data->name);
    return 0;
}

static void net_destroy_impl(void* context) {
    printf("  [NetIO] 切断\n");
}
...
`FileIO` を生成していた1行を `NetIO` の生成に書き換えるだけです。ビジネスロジック（`processor.c`）は1行たりとも修正していません。これこそが、私たちが多大なる労力をかけて「抽象化」と「依存性逆転」を行った最大の理由であり、OCP（開放閉鎖原則）が達成されたことの証明です。

#### main.c (既存のロジックに触れずに行う具象の切り替え - NetIOへの切り替え)
```c
#include "processor.h"
#include <stdio.h>
...
#### 実行結果
```c
=== LSP/OCP検証コードの実行 (NetIOへ切り替え) ===

[Main] NetIOモジュールを初期化完了

[Processor] 入力処理開始
  [NetIO] ネットワーク '192.168.1.10' からデータ受信
[Processor] データ解析開始
  [JsonParser 1回目] データ '{"id":99, "name":"NetworkUser"}' を解析中...
  [JsonParser 1回目] 解析完了: ID=20, Name=FileUser_A
[Processor] 出力処理開始
  [NetIO] ネットワーク '192.168.1.20' へ送信: ID=20, Name=FileUser_A
  [JsonParser 1回目] データ解放
[Processor] 処理完了
  [JsonParser] リソース解放（総解析回数: 1）
  [NetIO] 切断
...
基本となる **Simple Processor** の設計は完成しました。次章「第14章 統合実践（応用）」では、以下の課題に対し、第1部と第2部で習得した技術と原則を、より高度に統合し、具体的な実装パターンを通じて解決する方法を学びます。
