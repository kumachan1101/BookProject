...
依存性逆転原則(DIP)に基づき、`main`関数がコンポジションルートとして機能する実装です。すべての依存関係（`FileIO`, `JsonParser`）の生成と結合をここで行います。

**評価:** すべての依存関係を生成・注入し、システムを起動するコンポジションルートの実装です。
**設計ポイント:** すべての具象クラスの生成と接続を `main` 関数（コンポジションルート）一箇所に集中させることで、アプリケーション全体の依存関係構造を俯瞰できるようにし、個々のクラスから「他のクラスを生成する責任」を取り除いています。
#### main.c
```c
// システムのエントリポイント
...
**設計ポイント:** このスタブ実装は、`IIO` インターフェースの契約（関数シグネチャと振る舞い）さえ満たせば、内部で何を行っても構わないことを示しています。これが「リスコフの置換原則（LSP）」の本質であり、モックテストの基礎となります。
#### net_io_impl.c
```c
#include "i_io.h"
#include <stdlib.h>
...
`main.c`内の具象生成部分を変更するだけで、`FileIO`から`NetIO`へとシステム全体の振る舞いを切り替えることができます。

**評価:** 具象の生成部分を書き換えるだけで、ロジック本体には一切触れずに機能を拡張できています。
**設計ポイント:** `main.c` の一部（生成処理）を書き換えるだけで、システム全体の振る舞いが `FileIO` モードから `NetIO` モードへ完全に切り替わりました。`processor_run` というコアロジックは、自分が何を使っているかを知らないまま、正しく動作し続けています。
#### main.c
```c
#include "processor.h"
#include <stdio.h>
...
**[[18_第2部 第14章 統合実践（応用）_01|第14章 統合実践（応用） 複雑な実務課題への対処]]**では、以下の課題に対し、第1部と第2部で習得した技術と原則を、**より高度に統合**し、具体的な実装パターンを通じて解決する方法を学びます。
