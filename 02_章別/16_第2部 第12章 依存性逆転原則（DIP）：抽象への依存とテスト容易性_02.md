...
#### data_processor.c (変更の理由が異なる機能が同居するアンチパターン - 続き)
```c
// 具象実装2：ZIP（追加された具象詳細）
...
#### concrete_transforms.c (契約に透過的に従うZIPおよびNoOp変換の実装 - 続き)
```c
// --- ZIP実装 ---
...
この要件に対し、DIPの概念を持たずに実装すると、設定用の関数を通じて「どの通知手段を使うか」を示す特定の数値（例：1ならEmail、2ならSMS）と、「具象のポインタ」を強引に受け取るという、非常に脆い設計になりがちです。
...
#### user_service.c (具象のポインタとタイプを強引に渡すセッター - 続き)
```c
...
#### user_service.c (数値指定とポインタ変換に苦しむ利用者 - 続き)
```c
int main(void) {
...
#### user_service.c (実行中にいつでも抽象を交換できるセッター - 続き)
```c
...
#### main.c (システムを止めずに依存先を切り替えるセッター注入 - 続き)
```c
...
> [!NOTE] 読者の疑問：DIPとOCPって、結局やっていることは同じでは？
...
#### mock_database.c (契約を遵守してモックを生成するファクトリ - 続き)
```c
...
#### test_business_logic.c (外部環境に一切依存しない超高速なテストコード - 続き)
```c
