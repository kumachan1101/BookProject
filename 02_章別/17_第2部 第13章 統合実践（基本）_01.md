 ...
 **設計上の課題** : 多くの責任を一人で抱え込み、具象に依存してしまっている「神クラス（God Class）」のC言語版です。このコードは「動けばいい」という発想で作られており、将来の変更コストが全く考慮されていません。

リファクタリング前の、すべての責任が入り混じった巨大な実装ファイルです。具象モジュールのヘッダを直接インクルードしており、密結合の状態を体現しています。ここから依存関係をどう解きほぐしていくかが本章のテーマです。
...
一つの関数内に、ファイル読み込みという「物理的な操作」と、全体の「流れの制御」が混ざっており、どちらの事情が変わってもこの関数を修正しなければなりません。
...
責務2:  
 **ビジネスロジック** : データの解析を行っています。本来は別の層に任せるべきです。

さらに、JSONパーサーという別のモジュールの具象関数がここで呼ばれています。データ形式が変わるだけで、このプロセッサ関数の修正が必要になってしまいます。
...
責務3:  
 **出力処理** : ファイルへの書き込みを行っています。単一の関数が多すぎる責任を負っています。

エラーハンドリング（クリーンアップ処理）が関数内に散在しており、goto文を使って無理やりジャンプしています。これはリソースリークの温床となり、可読性も著しく低下させます。

出力の書き込みも同様です。このように「何をするか」と「どうやるか」が混然一体となっているのが、SOLID原則適用前の典型的な姿です。
...
最上位のデータ構造定義です。ここに変更があると全体に波及するため、安定依存の原則（SDP）に基づき、可能な限り変更が少なくなるよう設計する必要があります。複数のモジュール間で受け渡される「共通言語」としてのデータ構造定義です。このヘッダは上位から下位まで広く参照されるため、安定性を重視した設計が求められます。
...
操作の抽象インターフェース定義です。`IIO`構造体とVTableを通じて、具体的なIO処理を抽象化します。

IO操作の抽象化です。VTableを用いることで、ファイル、ネットワーク、メモリバッファなど、具体的な媒体の違いを利用者（Processor）から完全に隠蔽しています（Liskov Substitution Principle）。上位モジュール（Processor）が要求する「読み込み」「書き込み」という抽象的な操作を定義します。具体的な媒体（ファイル、DB、メモリ等）に依存しない契約を確立します。
...
抽象APIレイヤーを設けることで、クライアントコードはVTableの存在すら意識する必要がなくなります。また、ポインタのNULLチェックを一元化できるため、安全性も向上します。

抽象インターフェースを使いやすくするためのAPIを提供します。VTable経由での呼び出しを隠蔽し、安全なアクセスを保証します。これにより、クライアント側は関数ポインタを直接操作する必要がなくなります。
...
解析処理（Parse）とメモリ管理（Free）の責任をセットで定義しています。これは、生成したモジュールが解放の責任も持つべきという「対の原則」に従った設計です。データ解析の契約定義です。データの生成（parse）と破棄（free_data）の責任をセットで持たせることで、具象実装ごとのメモリ管理方針の差異を吸収します。
...
Parser用の抽象API実装です。解析結果（`ProcessedData`）の解放責任も、そのデータを生成したParser自身に負わせる「対の原則」を、この共通レイヤーで担保しています。
...
プロセッサモジュールは、具体的な機能（FileやJSON）ではなく、抽象的な役割（IIO, IParser）のみに依存しています。これにより、モジュールの独立性が高まり、単体テストが容易になります。全体の統括役としての契約です。具象クラスを一切参照せず、`IIO` や `IParser` という抽象名詞のみを用いてデータ処理の「流れ」を宣言します。
...
依存性の注入（DI）を利用して、実行時に振る舞いを決定しています。外部からオブジェクトを受け取ることで、プロセッサは「何を使うか」という関心事から解放され、「どう使うか」という純粋なロジックに集中できています。

全体の制御フローを司るコアモジュールです。IOやParserの具象クラスには一切触れず、抽象インターフェースのみを使ってビジネスロジックを組み立てています。このように上位モジュールを安定させるのがDIPの本質です。
...
ファイルIOを行う具象クラスのファクトリ関数宣言です。具象クラスの存在を隠蔽し、生成関数（Factory）のみを公開しています。

これにより、呼び出し側はヘッダファイルのインクルード依存を最小限に抑えることができます。具象実装の生成と抽象インターフェースへの「ワイヤリング」を行うための公開APIです。呼び出し側は、この具象モジュール内の詳細な構造体定義を知る必要はありません。
...
`i_io.h` の契約に従う必要があるため、関数シグネチャが強制的に統一されます。これにより、開発者は「何を実装すべきか」を迷う必要がなくなります。

ファイルシステムへの読み書きを具体的に実装します。ここでは、`i_io.h` という「抽象」が決めたルールに従ってコードを書くことが強制されます（DIP）。内部状態は `FileIOData` 構造体に閉じ込めています。
...
static int file_io_write_impl(void* self, const char* path, ProcessedData* data) {
    (void)self; // 警告抑制
    printf("  [FileIO] ファイル '%s' へ書き込み: ID=%d, Name=%s\n", path, data->id, data->name);
...
リソース解放の実装と、VTableの定義です。リソース解放 (`destroy`) もVTable経由で行うことで、呼び出し側は具象クラスの内部で何が `malloc` されたかを知る必要がなくなります。

完全にカプセル化されたメモリ管理です。リソースの破棄処理と、これら全ての操作をまとめたVTableの定義、そして具象インスタンスを生成するファクトリ関数を実装しています。

`destroy` メソッドも含めてVTableに登録することで、利用者が `free` を直接呼ぶのではなく、インターフェース経由で安全に破棄できるようにしています。また、ファクトリ関数内でデータ（`self`）と振る舞い（`vtable`）を結合（Dependency Injection）し、完全なオブジェクトとして返却します。

メモリ管理の責任がカプセル化され、利用者はオブジェクトの内部構造を知ることなく安全に生成・破棄が可能です。
...
JSON解析を行う具象クラスのファクトリ関数宣言です。具象クラスごとに専用のヘッダファイルを用意し、ファクトリ関数のみを公開するのは、C言語における「パッケージプライベート」な可視性を実現するテクニックです。

JSON解析器の生成関数です。将来的には、この同一ヘッダを使いながら初期化パラメータの異なる複数のパーサー（高速版、精密版など）を使い分けることも可能になります。
...
データ解放とリソース解放の実装、VTableの定義です。データ生成 (`parse`) と解放 (`free_data`) のメソッドが同じVTable内に同居しています。

これにより、アロケータ（malloc/free, new/delete, pool allocation）の不一致を防ぐことができます。

JSON解析の具体ロジックです。データの生成だけでなく、そのデータの責任を持って解放する `free_data` メソッドも実装します。これにより、生成と破棄のライフサイクルが具象側で完結します。
...
解析データの解放処理、パーサー自体の破棄処理、VTableの定義、およびファクトリ関数を実装しています。データの生成（`parse`）と解放（`free_data`）を同じモジュールに責務として持たせることで、メモリ確保と解放の方法（malloc/freeのペアなど）が一致することを保証しています。

リソース管理のライフサイクルがモジュール内で完結しており、メモリリークや誤った解放を防ぐ堅牢な設計です。
