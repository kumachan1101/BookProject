...
安定依存の原則（SDP: Stable Dependencies Principle）は、変更されにくい安定したモジュールに依存すべきという原則です。
...
そして、契約（VTable）で要求された「接続」「実行」「切断」の具体的な処理を、ファイル内部（`static`）で実装します。ここで `void* context` を元の具象型にキャストして操作するのがお決まりのパターンです。

#### mysql_adapter.c (契約メソッドを実装するMySQLアダプタ)
```c
// 具象実装
...
#### mysql_adapter.c (抽象インターフェースを生成するファクトリ（MySQL）)
```c
// VTableの実体
// 関数ポインタを静的に定義します
...
#### postgres_adapter.c (抽象インターフェースを生成するファクトリ（PostgreSQL）)
```c
// VTableの実体
// 関数ポインタを静的に定義します
...
#### main.c (利用する具象を選択して注入するエントリーポイント)
```c
int main(void) {
    // 1. MySQLの実装を注入
 ...
 **注入と利用の分離** : `main`（構成ルート）での依存性の注入フェーズと、上位のビジネスロジック（`OrderService`などの業務処理）での利用フェーズが完全に分かれています。

 **ロジックの不変性** : ビジネスロジックのコードは、接続先のDBがMySQLからPostgreSQLに変わっても一切変化しません（OCPを満たしています）。


```mermaid
    sequenceDiagram
    autonumber
    participant M as メイン (実行・注入者)
    participant Biz as ビジネスロジック (上位方針)
...
DIPを実際のコードに適用する際、「いつ・どのように」抽象（インターフェース）を渡すか（注入するか）によって、大きく3つのパターンに分類されます。それぞれのパターンは「注入のタイミング」と「適した用途」が異なるため、要件に合わせて正しく使い分けることが重要です。

以下に3つのパターンの全体像を示します。この違いをあらかじめ意識しながら、各パターンの詳細な実装手法を見ていきましょう。

| パターン名 | 注入するタイミング | 依存オブジェクトの寿命 | 主な用途・使い分けの基準 |
| ---- | ---- | ---- | ---- |
| **1. コンストラクタ注入**<br>（生成時注入） | オブジェクトの **生成時** に1回のみ | 親オブジェクトと同じ寿命を全うする | **【必須の依存関係】**<br>DB接続やロガーなど、最初から最後まで必ず存在し、途中で変わることがない動作の核心部分に使用します。 |
| **2. メソッド注入**<br>（関数引数注入） | 関数の **呼び出し時** （毎回） | 関数内での処理が終わるまで | **【一時的な依存・戦略の切り替え】**<br>データ変換アルゴリズムやソート処理など、「呼び出しごとに異なる振る舞いを適用したい」場合に便利です。 |
| **3. プロパティ注入**<br>（セッター注入） | オブジェクト生成後の **任意のタイミング** | セッターで上書きされるまで続く | **【任意の依存・動的な変更】**<br>ON/OFFを切り替える通知機能や、システムを止めずに稼働中に依存先を差し替えたい場合に有効です。 |
...
#### ❌ 原則適用前：具象クラスを直接生成し、特定のハードウェアに固執
...
#### weather_reporter.c (新しいデバイスが増えるたびに成長を続ける分岐地獄)
```c
// --- レポーター（複数の具象に密結合） ---
typedef struct {
...
利用側も、`0` なら物理ハードウェア、`1` ならモックといった特定の数値に振り回されていますし、初期化パラメータ（`param`）の意味すら分岐によって変わってしまうという危険な状態です。

#### weather_reporter.c (意味不明な数値指定に振り回される利用者)
```c
int main(void) {
...
#### ✅ 原則適用後：インターフェースを介した生成時注入による高いテスト容易性
...
// センサーロジックの型定義
typedef int (*ReadTempAction)(void* context);
...
static int hw_read_impl(void* context) {
    HardwareContext* ctx = (HardwareContext*)context;
    printf("[Hardware] Reading from I2C bus(0x%x)...\n", ctx->i2c_address);
    // 実機との通信処理が入る
    return 28;
}
...
static int mock_read_impl(void* context) {
    MockContext* ctx = (MockContext*)context;
    printf("[Mock] Returning fixed temperature...\n");
...
void destroy_sensor(ISensor* sensor) {
    if (sensor) {
        if (sensor->context) free(sensor->context);
        free(sensor);
    }
}
```
...
void report_weather(WeatherReporter* rep) {
    if (rep && rep->sensor && rep->sensor->read_temp) {
        ...
        int temp = rep->sensor->read_temp(rep->sensor->context);
        printf("Current Temperature: %d C\n", temp);
    }
}
```
