...
**設計ポイント:** マクロによって実装の詳細（VTableの接続や構造体の確保）が隠蔽され、利用者は「プロセッサを作成する」という意図だけに集中できます。これにより、定型的な実装ミスを防ぎつつ、可読性の高いクライアントコードを実現しています。
#### Client_Usage_Example.c
```c
IProcessor* enc = encryptor_processor_create();
char* result = enc->vtable->process(enc->self, "data");
...
**設計ポイント:** パイプラインの実装自体は、個々のプロセッサの具体的内容を知りません（オープン・クローズド原則）。`main`関数内でプロセッサの組み合わせや順序を自由に決定できるため、再コンパイルなしでシステムの振る舞いを構成変更（Configuration）レベルで制御可能になります。
...
**設計ポイント:** 用途（サムネイル、印刷、Web）ごとに異なる処理フローを、オブジェクトの再利用によって効率的に定義しています。各処理器は単一責任に徹しており、それらを組み合わせる責任はクライアント側（アプリケーション層）に委譲されています。
...
**別の例:** データ分析の分野でも、前処理（正規化、外れ値除去など）の順序を実験的に変える際、この設計が威力を発揮します。
**設計ポイント:** データサイエンスのワークフローでは、試行錯誤（Experimentation）が頻繁に行われます。パイプライン構築をコード（コンパイル単位）として分離することで、ロジック本体には影響を与えずに実験的なフローを即座に試せるアジリティが得られます。
...
**設計ポイント:** このような密結合は、単体テストを困難にし、システムの一部だけを切り出して再利用することを不可能にします。
...
**テスト失敗の例:** 以下のコードは、本物の暗号化ロジックが強制的に動いてしまうため、単体テストとして機能しません。
**設計ポイント:** `IEncryption`インターフェースを導入することで、`PaymentProcessor`は「暗号化する」という抽象的な振る舞いにのみ依存します。具体的なアルゴリズム（AES、RSAなど）は外部から注入（Dependency Injection）されるため、呼び出し側は詳細を知る必要がありません。
...
**設計ポイント:** コンストラクタ（作成関数）を通じて、具体的な実装である`AES_Encryption`を注入しています。これにより、`PaymentProcessor`を変更することなく、暗号化方式を将来的に変更したり、テスト用のダミー暗号化クラスに差し替えたりすることが可能になります。
...
**設計ポイント:** モックオブジェクト（`MockEncryption`）を使用することで、本物の暗号化ロジックを使用せずに`PaymentProcessor`の振る舞いだけをテストできます。これは、複雑な依存関係を持つシステムにおいて、テストの分離と高速化を実現するための定石です。
#### i_crypto_engine.h
```c
...
**設計ポイント:** VTableを模倣した構造体を使用し、関数ポインタを差し替えることで、実行時に振る舞いを変更できる動的なモックを実現しています。これがC言語における「インターフェースに対するプログラミング」の究極の形です。
...
抽象APIの実体です。VTableを経由して具象クラスのメソッドを呼び出すことで、多態性を実現します。
...
顧客モジュールの公開ヘッダです。`ICryptoEngine` への依存を `customer_module_create` の引数として明示的に宣言しています（生成時注入）。
...
顧客モジュールの実装です。`crypto_encrypt` などの抽象APIのみを利用することで、特定の暗号化実装への依存を排除しています。
...
AES暗号化エンジンの実体です。`ICryptoEngineVTable` を実装し、抽象インターフェースの契約を満たします。
...
テスト用のモックエンジンの宣言です。テストコードから状態を注入できるように、`MockCryptoData` へのポインタを受け取るファクトリ関数を提供します。
...
モックエンジンの実装です。実際の暗号化は行わず、呼び出し回数や引数を記録することで、テストコードによる検証を可能にします。
**設計ポイント:** 依存性の注入（DI）とインターフェース分離（ISP）を組み合わせることで、テスト容易性と拡張性を同時に達成しました。これは、大規模なCプロジェクトにおいて「腐らないコード」を維持するための重要な防波堤となります。
#### mock_crypto.c
```c
...
本番環境での利用例です。本物のAESエンジンを生成し、顧客モジュールに注入して使用します。
...
テスト環境での利用例です。モックエンジンを生成し、顧客モジュールに注入します。処理実行後、モックに記録された情報を検証することで、正しく暗号化処理が呼び出されたかを確認できます。
...
1. **Pipelineパターン**: 処理の順序や組み合わせが頻繁に変わるなら、マクロとVTableで「設定で動く」仕組みを作る。
2. **Context/Adapterパターン**: グローバル変数への依存を断ち切り、抽象インターフェース経由でリソースを注入することで、「テスト容易性」を確保する。

これらは、私がC言語開発で最も頻繁に使用し、最も助けられたパターンたちです。

### 次章への橋渡し

いよいよ第2部の最終章です。「原則」も「パターン」も学びました。しかし、知識として知っていることと、息をするように使いこなせることの間には壁があります。

[[19_第2部 第15章：SOLID原則を「使える思考」にする|第15章：SOLID原則を「使える思考」にする]]では、これまで学んだすべての知識を統合し、設計判断を下す際の「思考プロセス」そのものを言語化します。設計に迷ったとき、どのように考え、どの原則を選び取ればよいのか。その羅針盤をお渡しします。