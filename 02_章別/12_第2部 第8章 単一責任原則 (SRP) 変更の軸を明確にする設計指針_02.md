...
ここでは第2章で学んだ **関数ポインタ（Strategyパターン）** を用いて、「どのアルゴリズムを使うか（選択）」と「どう処理するか（実行）」という2つの責任を物理的に分離するパターンを実践します。選択基準の変更と計算ロジックの変更が互いに干渉しない、拡張に強い設計を構築します。

#### システム概要

ここでは、入力されたデータブロックのサイズに応じて、動的に最適な圧縮アルゴリズムを選択・実行する**データ圧縮システム**を実装します。具体的には、データサイズが1024バイト未満であれば「LZ4」アルゴリズムを使用し、1024バイト以上であれば「Gzip」アルゴリズムを使用して圧縮処理を行うという要件を持っています。

#### 設計課題

この要件を満たすため、まずはよくある素朴な実装を見てみましょう。以下の `compression_bad.c` では、データのサイズに基づく **選択ロジック** （ポリシー担当が決定権を持つ）と、各圧縮アルゴリズムの具体的な **計算ロジック** （圧縮担当が決定権を持つ）が1つの関数 `compress_data` の中に混在しています。

これはSRP違反の典型例であり、 **2つの異なるアクターが同じコードを変更する理由を持ってしまっている** 状態です。

| アクター | 変更要求 | 影響範囲 |
| --- | --- | --- |
| **ポリシー担当** | 選択基準を`1024`から`2048`に変更 | `compress_data`全体を修正・リビルド |
| **圧縮担当** | LZ4やGzipの計算パラメータを最適化等 | `compress_data`全体を修正・リビルド |

この設計のままでは、将来「Brotli」（Googleが開発した高圧縮率を誇るデータ圧縮アルゴリズム）などの第3のアルゴリズムを追加しようとした際にも、この `if-else` 分岐の中に直接手を入れなければならず、既存の（安定して動いていたはずの）LZ4やGzipのロジックにまで破壊的な影響を及ぼす危険性があります。


#### ❌ 原則適用前：選択ロジックと計算ロジックが混在したコード
#### compression_bad.c
```c
#include <stdio.h>
#include <string.h>
...
クライアント側から見ると、単に「圧縮してほしい」と依頼しているだけですが、内部ではデータサイズによる閾値判定がハードコーディングされており、ポリシー変更（閾値の変更など）が起きた際に、この関数に依存するすべてのコードが影響を受ける壊れやすい構造になっています。

#### compression_bad.c (ハードコードされた分岐に依存するクライアント)
```c
int main(void) {
    printf("=== SRP違反コードの実行 ===\n\n");
...
「選択基準」を変えたい人と「計算方法」を変えたい人が、同じファイルを編集することになり、 **変更の衝突** が発生しやすくなります。
...
この「選択基準の変更」と「計算ロジックの変更」の衝突を防ぐための設計手法が、 **第2章で解説した関数ポインタ（Strategyパターン）を用いた分離** です。

「何を実行するか（計算ロジック）」はLZ4専用、Gzip専用の独立した別々のファイルに記述します。これらは共通の関数ポインタ `CompressFunc` という抽象的な契約（インターフェース）を満たすように作られます。そして「どれを使うか（選択ロジック）」のファイルには、具体的に圧縮を行うコードは一切書かず、条件に基づいて適切な関数ポインタを返す役割だけを持たせます。最後に全体の制御を行うファイルは、選ばれた関数ポインタを「ただ実行する」だけです。これにより、アルゴリズムが増えても選択ロジックが変わっても、互いのファイルに一切影響を与えない（再コンパイルが不要な）設計が可能になります。

#### ✅ 原則適用後：計算ロジックを分離し再利用性を高めた構成
...
#### compression_strategy.h (実行の責任を抽象化する契約定義)
```c
...
#### lz4_strategy.h (実装の詳細を隠蔽するファクトリ公開)
```c
...
#### lz4_strategy.c (単一のアルゴリズムに専念する計算モジュール)
```c
...
#### gzip_strategy.h (Gzipアルゴリズムのファクトリ公開)
```c
...
#### gzip_strategy.c (Gzipの計算ロジックのみに責任を持つ実装)
```c
...
#### compression_selector.h (戦略の選択基準を公開するインターフェース)
```c
...
#### compression_selector.c (ポリシーの関心事のみを集約した選択モジュール)
```c
...
#### compressor.h (利用者へ提供する抽象的な圧縮の窓口)
```c
#ifndef COMPRESSOR_H
#define COMPRESSOR_H
...
#### compressor.c (選択と実行のコーディネートに徹する制御モジュール)
```c
...
「選択(Selector)」「契約(Interface)」「実装(Strategy)」の **3層構造** に綺麗に分かれています。
...
ここでは、書籍のタイトルとISBN番号を管理する**簡易的な図書管理システム（Library）** を実装します。このシステムは、新しい書籍データを動的にメモリへ追加・保存していく「状態管理」の要件と、現在登録されている全書籍のリストを整形して画面へ出力する「状態表示」の要件を持っています。

#### 設計課題

この要件に対し、以下の `library_bad.c` では `Library` 構造体とそれに付随する関数群が、 **書籍リストの管理（メモリ確保・整合性維持）** と **画面表示（標準出力への文字列整形）** という、本来分離されるべき2つの責任を同時に抱え込んでしまっています。

結果として、 **2つの全く異なるアクターが同じコード（ライブラリの中核）を変更する理由を持ってしまっている** という致命的な状態（SRP違反）に陥っています。

| アクター | 変更要求の例 | 影響範囲 |
| --- | --- | --- |
| **状態管理担当** | 書籍の重複登録を禁止したい、検索を高速化したい | `library_bad.c` のロジック全体を修正・検証 |
| **UI担当** | 表示をリスト形式からJSON形式に変えたい、GUI出力に変えたい | `library_bad.c` のロジック全体を修正・検証 |

データの持ち方（状態管理担当の要求）を変えると表示ロジックも壊れ、一方で出力をJSON形式に変えたい（UI担当の要求）と思うだけで、全く関係のないメモリ管理コードの動作保証テストまでやり直さなければならない、非常にもろく保守性の低い設計です。しかも `print_status` 関数が内部配列に直接アクセスしているため、将来管理構造をハッシュマップや木構造に変更することが事実上不可能です。
#### ❌ 原則適用前：計算と通知(I/O)が密結合した命令的な処理
#### library_bad.c
```c
#include <stdio.h>
#include <stdlib.h>
...
#### library_bad.c (内部データ構造へ直接依存する表示ロジック)
```c
...
--- Library Status ---
Total: 1 books
[1] Clean Code (ISBN: 978-0131103627)
```
...
この「ドメイン知識（状態管理）」と「プレゼンテーション（画面表示）」の癒着を引き剥がすのが、 **MVC（Model-View-Controller）パターンのModelとViewの分離** です。

「本をどう記憶するか（Model）」はLibrary専門のファイルに閉じ込め、表示用関数を持たせません。「本をどう見せるか（View）」はUI専門のファイルに書き、内部配列への直接アクセスを禁じて専用の取得関数（ゲッター）経由でのアクセスのみを許します。これにより、UIの要件がどれほど複雑化（HTML/JSON等）しようとも、システムの心臓部である状態管理ロジックには一切手を入れる必要がなくなる（変更の波及を完全に遮断できる）堅牢な設計が完成します。

#### ✅ 原則適用後：責任の分割(通知を抽象化し計算を純粋に保った設計 (SRP準拠))
| モジュール | 責任 | 変更を要求するアクター |
| --- | --- | --- |
