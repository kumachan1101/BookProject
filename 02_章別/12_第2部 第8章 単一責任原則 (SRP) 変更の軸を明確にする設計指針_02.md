...
`Library` 構造体の内部データ（`books`配列）に直接アクセスし、標準出力にリスト形式で表示する関数です。（悪い例）状態管理を行うファイル内に表示ロジックが含まれており、責任が混在しています。
 ...
 **このファイルの目的** : システムの基盤となるデータ構造（Value Object）を定義し、各モジュール間の共通言語として機能させます。書籍データ（タイトル、ISBN）を表す構造体 `Book` を定義しています。
