...
> [!INFO] `self` ポインタの意味について
> ここで、VTableの実体を受け取る関数の第一引数に `void* context` ではなく `INotifyStrategy* self` を指定している点に注目してください。
> 第1部で学んだように、内部の不透明な「状態・設定データのみ」を渡したい場合は `void* context` としますが、ここでは「インターフェース自体（`vtable` を持つ構造体）」を自分自身のインスタンスとして受け取っています。これはオブジェクト指向における `this` や `self` の直訳であり、「自身の振る舞いと状態をまとめた完全なオブジェクトへのポインタ」を意味します。
...
> [!NOTE] 読者の疑問：つまり、具体を生成する工場の役割以外に、ifやswitchを使っていたら、OCPに反する傾向があるという見方をすればよいですか？
> 
> **回答：**
