...
フラグによる分岐を用いた通知送信の実装です。`bool` フラグを受け取り、if文で処理を分岐させて、即時送信か予約送信かを決定しています。

（悪い例）単純な条件分岐で要件を満たそうとしています。短期的には実装が早いですが、拡張性を考慮していません。

送信モードが増えるたびに `send_notification` 関数内のif文が増殖し、テストと保守が困難になります**（OCP違反）**。
...
通知送信の抽象インターフェース定義です。送信操作（`send`）の関数ポインタを持つVTableと、それを保持する戦略インターフェース（`INotifyStrategy`）を定義しています。

通知の「送信」という振る舞いを抽象化し、具体的な実装（即時、予約など）を隠蔽して、呼び出し側から切り離します。新しい送信手段を追加しても、このインターフェース定義は不変であり、既存の利用コードに影響を与えません。
...
即時送信を行う具体的な戦略の実装です。`INotifyStrategy` インターフェースに従い、即時送信（コンソール出力）を行う具体的な処理を実装しています。

独自のデータを持たないシンプルな戦略であるため、基底構造体 `INotifyStrategy` をそのまま使用し、VTableのみを差し替える設計です。最低限のメモリ確保で済む効率的な実装であり、インターフェースの契約を正確に履行しています。
...
予約送信を行う具体的な戦略の実装です。基底構造体 `INotifyStrategy` を先頭に配置することで、ポインタのキャストを安全に行い、C言語における継承と多態性を実現しています。

`INotifyStrategy` を拡張した構造体を定義し、指定時刻に行う予約送信のロジックを実装しています。基底構造体（`base`）を構造体の先頭に配置することで、C言語でも安全にアップキャスト（親クラスへの変換）が可能になり、多態性を実現しています。

固有データ（`scheduled_time`）を持ちながらも、外部からは `INotifyStrategy` として統一的に扱える優れた拡張設計です。
...
予約送信のインスタンスを生成するファクトリ関数です。予約送信用のインスタンスをヒープに確保し、VTableと固有データ（予約時刻）を初期化して返却します。

インスタンス生成の複雑さを隠蔽し、常に初期化済みの安全なオブジェクトを提供します。戻り値を抽象型（`INotifyStrategy*`）とすることで、呼び出し側から具象型を隠蔽しています。

メモリ確保と初期化の責任が一箇所に集中しており、利用側の負担を軽減しています。
...
通知戦略を生成し、注入して実行するクライアントコードです。具体的な戦略（即時、予約）を生成し、`send_notification` に渡します。

依存性の注入（DI）を行い、使用する戦略を決定します。実行時に柔軟に戦略を切り替えられます。
...
if文を消滅させることはできません。OCPの真の狙いは、if文をビジネスロジックから排除し、 **「生成（Factory）」という特定の汚れた場所に隔離すること** です。

#### ファクトリ関数の実装例

#### device_factory.c

文字列キーを使って適切なデバイスインスタンスを生成するファクトリです。文字列比較により、対応する具象デバイスの生成関数を呼び出します。

分岐（if文）をこの１か所に集約し、ビジネスロジックから排除します。新しいデバイス追加時の修正箇所がここだけに限定されます。
