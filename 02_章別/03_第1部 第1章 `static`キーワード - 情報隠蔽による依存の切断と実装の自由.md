...
本書の **第1部「基礎道具編」** において、 **`static`キーワードは、C言語のモジュール設計における最も基本的な「依存を切断するための道具」** として機能します。 **C言語** では、多くの要素がデフォルトで **外部リンケージ** （グローバルな公開）を持ってしまいます。これが意図せずモジュール間に **強い結合** （Tight Coupling）を生み、結果として **変更容易性** を著しく損なう原因となるのです。
 ...
 **`static`（道具）** → **単一責任原則 (SRP)の基礎（原則）** → **保守性/変更容易性の向上（目的）**
...
温度センサーの値を読み取る組み込みモジュールにおいて、内部状態（キャリブレーション値やエラーカウント）を外部から隠蔽し、安全に管理する。

- **センサーの初期化**
- **現在温度の読み取り（補正済み）**
- **内部状態を持つシングルトン的なモジュール構成。**

APIを通じて初期化指示と温度値（整数）をやり取りする。
...
内部状態に関する変数宣言や構造体定義を一切含めていません。これにより、実装詳細（キャリブレーション値の管理方法など）が変更されても、このヘッダファイルを利用する外部モジュールへの影響（リコンパイル等）を最小限に抑えています。

必要最小限の情報のみを公開する、「 **結合度の低いヘッダ設計** 」の好例です。
...
内部の検証ロジック（`is_reading_valid`）や補正値（`s_calibration_offset`）を隠蔽することで、将来的に検証ルールや補正方法が変わっても、このAPIを利用する外部コードには影響を与えないようにしています。これが「変更の影響範囲を局所化する」ということです。

呼び出し側は「温度を得る」という目的だけに集中でき、内部ロジックの複雑さから解放されています。
...
センサー内部のキャリブレーション方法やエラー変数の存在を全く意識せずに利用しています。これにより、`temperature_sensor.c` 側の実装変更に対して疎結合な状態が保たれています。

モジュール間の依存関係が単純化されており、「 **仕様変更に強い構成** 」となっています。
...
アプリケーション全体で使用する簡易ログシステムにおいて、明示的な初期化関数を呼ばずに、初回利用時に自動で初期化を行う。
...
状態変数のスコープを関数内に極限まで狭めています（カプセル化）。これにより、他の関数から誤って初期化フラグやカウンタが変更されるリスクをゼロにしています。

グローバル変数を使わずに状態保持を実現する、安全で保守性の高いパターンと言えます。
...
#### 設計の意図
リソースの競合や不整合を防ぐため、インスタンスの一つだけの存在を保証します。また、`static` 変数を利用してインスタンスを隠蔽し、グローバル変数の弊害（どこからでも変更可能）を回避しつつ、必要な箇所からのみアクセスできる制御された共有を実現します。
...
データベースコネクションのような高コストなリソースを、システム全体で一つだけ存在するよう制御し、複数の初期化を防ぎたい。複数のモジュール（Caller1, Caller2）が、`connection_pool_get` APIを通じて、唯一の実体（Instance）を共有する構造です。
...
複数のモジュールが同じインスタンスにアクセスするが、実体は一つのみ存在する。

#### connection_pool.h

コネクションプールのインターフェース定義です。今回は構造体の隠蔽（不完全型）も利用しています。

`ConnectionPool` という不完全構造体（タグ名のみ）を typedef し、取得 API `connection_pool_get` を宣言しています。構造体の中身を見せないことで、利用側が勝手にメンバを書き換えることを防ぎ、必ず API を通して操作させるよう強制しています。

カプセル化が徹底されており、非常に堅牢なモジュール設計です。
...
`s_pool` を `static` にすることで、このファイル外部から直接アクセスできないようにし、必ず `get` 関数を通すように制御しています。これにより、インスタンスの唯一性と初期化の保証を行っています（シングルトン）。

グローバル変数による安易な共有ではなく、アクセス制御された共有を実現しています。マルチスレッド環境での排他制御が必要な点には注意が必要ですが、基本的なシングルトンの実装として正しいアプローチです。
...
どのタイミングで誰が呼んでも、常に「初期化済みの唯一のインスタンス」が返されることを期待したコードです。システム全体でのリソース共有をシンプルに行っています。

シングルトンの利点を活かし、リソース管理を一元化できています。
...
`static`は強力な **情報隠蔽の道具** ですが、設計的判断と **トレードオフ** が伴います。`static`キーワードを使用する際のメリット、デメリット、およびリスクのバランスです。
 ...
 **対策**: 共有される`static`データへの変更を行う公開APIには、 **ミューテックス** による排他制御を導入します。

または、 **スレッドローカルストレージ** の使用を検討します。

#### 情報隠蔽の徹底

 **設計的判断（トレードオフ）** : モジュール内の補助的なデータやヘルパー関数のうち、一つでも`static`が漏れてしまうと、そこが「勝手口」となり、意図しない **依存** を招く原因になります。 

 **対策**: ヘッダファイルで公開していない要素には、 **「原則としてすべてstaticを付ける」** という習慣を持つことが有効です。

これにより、意図しない外部露出を確実に防ぐことができます。
