...
> [!NOTE] 読者の疑問：なぜわざわざ変数を隠す必要があるのか？
> 「名前の衝突さえ気を付ければ、どこからでもアクセスできるグローバル変数の方が便利なのでは？」
>
> C言語の初心者が設計を学ぶ際、最初に抱く疑問がこれです。しかし、実務の現場では、この「隠さない」という選択がしばしば致命的な悲劇を生みます。例えば、システム全体からアクセス可能なグローバル状態を、ある開発者が便利だからといって直接書き換えてしまった結果、全く関係ない別の機能で原因不明のエラーが発生し、徹夜で何十個ものソースファイルを `grep` してバグの発生源を追いかける羽目になる……といった事態です。

本書の **第1部「基礎道具編」** において、 **`static`キーワードは、C言語のモジュール設計における最も基本的な「依存を切断するための防壁」** として機能します。 **C言語** では、多くの要素がデフォルトで **外部リンケージ** （グローバルな公開）を持ってしまいます。これが意図せずモジュール間に **強い結合** （Tight Coupling）を生み、上述のような悪夢を引き起こす原因となるのです。
...
モジュールが自身の内部状態に対して全ての **責任** を持ち、外部からの干渉を一切許さない構造を作る。これこそが、徹夜のデバッグを無くし、 **変更の影響範囲** を限定し、真の **疎結合** を実現するための土台となります。
...
*   **コンパイル依存の最小化**
    `static`にされた要素はヘッダファイルに記述する必要がありません。その実装を変更しても、他のファイルを **再コンパイル** する必要がなくなるため、コンパイル時の **依存関係が物理的に切断** されます。

*   **実装の隠蔽と自由**
    外部から見えないため、`static`な実装はいつでも **自由に変更・削除** できます。これにより、モジュールの **保守性** が劇的に向上します。
...
*   **API**
    唯一の「窓口」として機能し、内部状態を保護しています。
*   **State/Helper**
    モジュール内部に閉じており、外部からの変更影響を受けません。
...
*   **State**
    関数内`static`変数として定義されており、関数外からは見えません。
*   **Logic**
    呼び出しごとに状態（回数や初期化フラグ）を更新します。
...
**シングルトンパターン (Singleton Pattern)** とは、プログラム実行中に特定のクラスや構造体のインスタンスが「絶対に一つしか存在しない」ことを保証する設計パターンのことです。リソースの競合を防いだり、メモリを節約したりするために使われます。

#### 解決する設計課題

データベースコネクションのような高コストなリソースを、システム全体で一つだけ存在するよう制御し、複数の初期化を防ぎたいという課題に対処します。複数のモジュール（Caller1, Caller2）が、`connection_pool_get` APIを通じて、唯一の実体（Instance）を共有する構造です。

*   **Instance**
    `static`変数として定義された唯一の実体です。
*   **Init**
    初回アクセス時のみ初期化が走るガード機能です。
