...
メモリ管理に関する「誰が」「いつ」という根本的な疑問と、それに対する設計上の解答です。

**注目ポイント**:

* **責任**: 「確保した人」ではなく「所有者」が解放責任を持つ、という点が重要です。
...
これら三つの問いに対する答えを、**コードレベルで明確にする技術**を習得します。
...
* **責任の移動**: 関数呼び出し後は、呼び出し元はもうそのリソースに触れてはいけません。
...
呼び出し側が所有権を保持したまま、関数に「貸し出す」パターンです。C言語で最も頻繁に使われます。
...
* **所有権の移動**: 関数呼び出し以降、呼び出し側はリソースに触れていない点（**Dangling（ダングリング）ポインタ**防止）。※ダングリングポインタとは、解放済みの無効なメモリ領域を指しているポインタのことです。

**読み方のガイド**:

* 時間軸（上から下）に沿って、所有権の所在がCからFへ移る瞬間を確認してください。
...
参照パターンにおける安全な貸し借りのシーケンスです。

**注目ポイント**:

* **C (Caller)**: 呼び出し側のモジュール。
* **F (Callee)**: 呼び出される関数。
* F（関数）での処理が終わった後、C（呼び出し側）が責任を持って `free` しています。
...
**注目ポイント**:
* **責任の移動**: 関数の戻り値としてリソースが返され、その管理責任が呼び出し側に課せられます。
...
関数の目的に応じて、どの所有権パターンを採用すべきかの判断基準です。

**注目ポイント**:

* **目的**: 「作る」「使う」「捨てる」というアクションに合わせて形態を選びます。
...
* **隠蔽**: 内部リソースの管理責任を `destroy` 関数に一任することで、呼び出し側の負担を減らします。
...
`dm_create` でのリソース確保と、`dm_destroy` での解放が完全に対称になっています。
まず、構造体と生成関数です。

**処理の内容:** 内部構造体とリソースを確保・初期化します。

**設計的意図:** `dm_create` が内部でどのようなリソース（ここでは`int* data`）を確保したかを知っているのは `dm_create` だけです。したがって、後始末のやり方を知っているのもこのモジュールだけです。
...
**処理の内容:** 内部リソースと構造体本体を、確保とは逆の順序で解放します。
...
**処理の内容:** データを参照して処理を行います。解放はしません。

**設計的意図:** 参照渡し（Borrow）の実装です。
...
「生成」して「使用」して「破棄」するという一連のフローが、明確なAPIによって安全に行われています。
...
**実行結果:** ファクトリ関数内でリソースが確保（内部配列含む）され、使用後にデストラクタで一括解放されている様子が確認できます。
...
* **スタック利用**: 呼び出し側がスタック変数を使えるため、`malloc` のコストを回避できます。
...
関数はバッファのポインタとサイズを受け取るだけで、メモリ確保は行いません。
...
実装側では、渡されたバッファサイズを超えないように安全に書き込むことだけに集中します。
...
`goto cleanup` を使うことで、正常系の処理フロー（一直線）と、異常系の処理フロー（共通の出口へジャンプ）を分離した構造です。
...
**処理の内容:** コンパイルを通すための型と関数を定義しています。

**設計的意図:** サンプルコードの簡潔化。

**評価:** 本質的なロジックに集中させるための準備です。
...
次に、本題の `goto cleanup` を用いた関数です。

**処理の内容:** 変数を初期化し、順番に確保し、失敗したら `cleanup` へ飛びます。最後は逆順に解放します。

**設計的意図:** エラー処理を一元化し、解放漏れを防ぎます。特に「初期化でNULLを入れる」「逆順解放」「NULLチェック」の3点が肝です。

**評価:** C言語におけるリソース管理のベストプラクティスです。
