...
まずは計算ロジックを抽象化するインターフェース（契約）です。
「税金を計算する」という振る舞いだけを定義し、日本やアメリカといった具体的な国名や計算ルールは一切登場させません。

#### itax.h (税計算の振る舞いだけを定義した抽象契約)
```c
typedef struct {
...
次に、**「本物の税計算ロジック（`japan_tax`など）」が影も形もない状態** で、注文の合計金額を計算するビジネスロジックとそのテストを書いてしまいます。

ここで活躍するのが関数の内部で作る「10%固定」の簡易モックです。これを注入することで、本番の実装を待つことなく `calculate_total` 関数のロジックが正しいこと（単体テスト）を100%証明できます。これがDIP＋モックの真骨頂です。

#### test_order.c (本物の実装を待たずにロジックを証明するテスト)
```c
#include "itax.h"
#include <assert.h>
#include <stdlib.h>
...
double calculate_total(double amount, ITax* tax) {
    ...
    // 契約に従って計算を委譲
    return amount + tax->vtable->calculate_tax(tax->context, amount);
}
...
static double mock_calc(void* context, double amount) {
    MockTax* m = (MockTax*)context;
...
「全てのテストが成功しました！」という出力は、途中の `assert` 検証がすべてパスし、プログラムが最後まで正常に到達したことを示しています。

### ステップ3：本物の実装

テストが通り、ビジネスロジックの正しさが証明された後で、おもむろに本物の「日本の税計算」ロジックを実装します。

出来上がったこの本物のファクトリ関数（`create_japan_tax`）を、本番環境の `main` 関数で注入するだけで、システムは完成です。テストコードもビジネスロジックも、1行たりとも書き直す必要はありません。

#### japan_tax.c (契約に従い後から追加される本物の実装)
```c
#include "itax.h"
#include <stdlib.h>

static double japan_tax_calc(void* context, double amount) {
    (void)context; // 日本の税率は固定（10%）
    return amount * 0.10;
}
...
#### 誤解1：「すべてを抽象化すべき」 (よくある誤解と注意点)
   
何でもかんでもインターフェースを作って抽象化すれば良いわけではありません。抽象化は強力な設計の武器ですが、コストや複雑さも伴います。実際に抽象化を適用すべきなのは、以下のようなケースに限られます。
* 複数の実装が存在する（または将来存在する可能性が高い）
* テスト時にモックと差し替える必要がある
* 変更が頻繁に発生する不安定な部分である
...
関数ポインタ（VTable）を経由する動的な呼び出しには、確かに数ナノ秒程度のオーバーヘッドが存在します。さらに、コンパイラによる「インライン展開」といった最適化を妨げてしまう性質があるため、非常に小さな関数をタイトなループの中で何百万回も呼ぶような特殊なケースでは、パフォーマンスに影響が出るのは事実です。
しかし、実際の多くのアプリケーションにおいて、真のボトルネックとなるのは関数呼び出しではなくI/O操作（データベース通信やファイル書き込みなど）です。そのため、大部分のユースケースにおいては、オーバーヘッドによる軽微なタイムロスよりも、DIPを適用することで得られる「設計の柔軟性」や「テストの容易性」という恩恵の方がはるかに大きく勝ります。

#### 誤解3：「抽象は実装の詳細を一切知らない」

「インターフェースは具体的な処理を全く知らないのだから、どう使ってもいいはずだ」というのはよくある誤解です。抽象化されたインターフェースであっても、その背後にある実装が必ず守らなければならない「一般的な制約」や「暗黙のルール」を反映させることがあります。

例えば、ファイルシステムの抽象化を考えてみましょう。ファイルの保存先がNTFSであれext4であれネットワークストレージであれ、「まずはファイルを開き、次にデータを書き込み、最後に必ず閉じる」という生命周期や順序のルールは共通しています。こうした普遍的な手順や制約は、個別の実装に任せるのではなく、抽象化された契約（インターフェース）の一部としてあらかじめ定義しておくのが自然な設計です。

#### FileSystem_Example.h (操作の順序制約を表現した抽象インターフェース)
```c
// 例：ファイルシステムの抽象化
typedef struct {
 ...
 * **ワイヤリング** ：Main関数（コンポジションルート）で、別々に作られた部品同士を繋ぎ合わせるように、具象オブジェクトを生成・注入（ワイヤリング）していますか？ 
...
#### 次章への橋渡し
次章「第13章 統合実践（基本）」では、これまで学んだ原則（SRP, OCP, LSP, ISP, DIP）のすべてを統合し、実際のコードリファクタリングを通じて美しい設計へと作り変えていくプロセスを体験します。
